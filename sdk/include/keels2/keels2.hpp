...
protected:
    const Context& HostContext() const noexcept
    {
        return context_;
    }
    ...
    bool GetPlayerInput(const PlayerConnection& player, PlayerInput& input)
    {
        ...
        {
            input = {};
            return status_.Set(KEEL_RESULT_ENGINE_FAILURE, "could not read player input");
        }
    }
...
class AuthoringAdapter final
{
    ...
    struct State final
    {
        ...
        const KeelPauseApi* pause{};
        const KeelUnloadApi* unload{};
...
private:
    ...
    static KeelBool PreparePauseDispatch(void* user_data) noexcept
    {
        ...
        {
            instance->LogError("exception escaped pause preparation");
            return KEEL_FALSE;
        }
    }

    static bool PreparePauseCallback(Type& instance, State& state) noexcept
    {
        if constexpr (!kPreparePause)
        {
            return true;
        }
        const void* raw{};
    ...
    static void ReleasePauseCallback(Type& instance, State& state) noexcept
    {
        if (state.pause)
        {
    ...
    static KeelBool PrepareUnloadDispatch(void* user_data) noexcept
    {
        ...
        {
            instance->LogError("exception escaped unload preparation; plugin retained");
            return KEEL_FALSE;
        }
    }

    static bool PrepareUnloadCallback(Type& instance, State& state) noexcept
    {
        if constexpr (!kPrepareUnload)
        {
            return true;
        }
        const void* raw{};
    ...
    static void ReleaseUnloadCallback(Type& instance, State& state) noexcept
    {
        if (state.unload)
        {
