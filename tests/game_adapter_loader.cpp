...
int main(int argument_count, char** arguments)
{
    ...
    if (module.EntityConstruction().size || module.EntityConstruction().ready)
        return 20;

    if (module.EntityInput().size || module.EntityInput().dispatch)
        return 21;

    if (module.EntityOutputs().size || module.EntityOutputs().start)
        return 22;
    ...
    if (module.VisitEntities(
            &access,
            1,
            [](void* data, void* const*, std::uint32_t)
            {
                *static_cast<bool*>(data) = true;
                return KEEL_RESULT_OK;
            },
            &accessed) != KEEL_RESULT_UNSUPPORTED ||
        accessed)
        return 16;
    ...
    const KeelPlayerManagementAction management{
        sizeof(KeelPlayerManagementAction), KEELS2_PLAYER_MANAGEMENT_RESPAWN, 0, 0};
