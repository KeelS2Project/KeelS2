...
void* CreateEntityFixture(const char* name, int forced_index)
{
    ...
    if (g_construction_source) g_construction_source[0] = 'X';
    g_construction_name = name;
    if (g_construction_mode == 1) return nullptr;
    if (g_construction_mode == 2) return reinterpret_cast<void*>(1);
    if (g_construction_mode == 3) throw std::runtime_error("factory failure");
...
void SpawnEntityFixture(void* entity, const void* key_values)
{
    ...
    if (g_construction_mode == 4 || g_construction_mode == 5) {
        ...
        if (entity != EntityInstance()) ::operator delete(entity,std::align_val_t{16});
        else g_entity_storage.fill(std::byte{0xdd});
    }
    if (g_construction_mode == 5) throw std::runtime_error("spawn callback failure");
    if (g_construction_mode == 6) SetHandle(*Identity(),13);
}
void CancelEntityFixture(void* entity)
{
    ...
    if (g_construction_mode == 5) throw std::runtime_error("remove callback failure");
}
int RunEntityConstructionChecks()
{
    ...
    const auto capture = [&] { return KeelCs2_CaptureCreatedEntity(EntitySystem(),instance,&entity); };
    ...
    if (create(name) != KEEL_RESULT_OK || !g_construction_arguments || g_construction_name != "prop_dynamic" || name[0] != 'X' ||
        capture() != KEEL_RESULT_OK || entity.index != kEntityIndex || validate() != KEEL_RESULT_OK ||
        KeelCs2_ValidateEntity(EntitySystem(),&entity) != KEEL_RESULT_NOT_FOUND) return 1101;
    g_construction_source = nullptr;
    std::string longest(127,'a');
    if (create(longest.c_str()) != KEEL_RESULT_OK || g_construction_name != longest) return 1102;
    longest.push_back('a');
    const auto creates = g_create_calls;
    for (const char* invalid : {"","bad class","bad;class","bad/class","bad\nclass",longest.c_str()})
        if (create(invalid) != KEEL_RESULT_INVALID_ARGUMENT || instance) return 1103;
    if (g_create_calls != creates || KeelCs2_CreateEntity(&bindings,nullptr,&instance) != KEEL_RESULT_INVALID_ARGUMENT || instance ||
        KeelCs2_CreateEntity(&bindings,"prop_dynamic",nullptr) != KEEL_RESULT_INVALID_ARGUMENT) return 1104;
    auto incomplete = bindings; incomplete.remove = nullptr;
    if (KeelCs2_CreateEntity(&incomplete,"prop_dynamic",&instance) != KEEL_RESULT_INVALID_ARGUMENT || instance) return 1105;
    for (unsigned mode : {1u,2u,3u}) {
        g_construction_mode = mode;
        const auto result = create();
        if (result != (mode == 1 ? KEEL_RESULT_NOT_FOUND : mode == 3 ? KEEL_RESULT_ENGINE_FAILURE : KEEL_RESULT_OK)) return 1106;
        if (mode == 2) {
            entity = {123,456};
            if (capture() != KEEL_RESULT_NOT_FOUND || entity.index || entity.source2_handle) return 1107;
        } else if (instance) return 1108;
    }
    g_construction_mode = 0;
    if (create() != KEEL_RESULT_OK || capture() != KEEL_RESULT_OK) return 1109;
    ...
    if (validate() != KEEL_RESULT_NOT_FOUND || spawn() != KEEL_RESULT_NOT_FOUND || invoked) return 1111;
    SetHandle(*Identity(),12);
    for (int index : {-1,MAX_TOTAL_ENTITIES,kEntityIndex+1}) {
        entity.index = index;
        if (validate() != KEEL_RESULT_NOT_FOUND) return 1112;
    }
    entity = saved; entity.source2_handle = INVALID_EHANDLE_INDEX;
    if (validate() != KEEL_RESULT_NOT_FOUND) return 1113;
    entity = saved; EntityInstance()->m_pEntity = nullptr;
    if (validate() != KEEL_RESULT_NOT_FOUND || capture() != KEEL_RESULT_NOT_FOUND) return 1114;
    EntityInstance()->m_pEntity = Identity(); entity = saved;
    Identity()->m_pInstance = reinterpret_cast<CEntityInstance*>(g_entity_storage.data()+1);
    if (validate() != KEEL_RESULT_NOT_FOUND) return 1115;
    Identity()->m_pInstance = EntityInstance();
    g_derived_bases[0].m_nOffset = 8;
    if (validate() != KEEL_RESULT_INCOMPATIBLE || spawn() != KEEL_RESULT_INCOMPATIBLE || invoked) return 1116;
    g_derived_bases[0].m_nOffset = 0;
    g_base_class.m_pszName = "WrongBase";
    if (validate() != KEEL_RESULT_INCOMPATIBLE) return 1117;
    g_base_class.m_pszName = "CBaseEntity";
    if (validate(static_cast<KeelBool>(7)) != KEEL_RESULT_INVALID_ARGUMENT || g_spawn_calls || g_cancel_calls) return 1118;
    ...
    if (spawn() != KEEL_RESULT_NOT_FOUND || invoked || g_spawn_calls != 1) return 1120;
    if (cancel() != KEEL_RESULT_OK || !invoked || !g_construction_arguments || cancel() != KEEL_RESULT_NOT_FOUND || invoked ||
        g_cancel_calls != 1) return 1121;
    for (unsigned mode : {4u,5u,6u}) {
        Reset(); g_construction_mode = mode;
        if (create() != KEEL_RESULT_OK || capture() != KEEL_RESULT_OK) return 1122;
    ...
    if (create() != KEEL_RESULT_OK || capture() != KEEL_RESULT_OK) return 1125;
    ...
    for (unsigned mode : {4u,5u}) {
        for (bool spawning : {false,true}) {
            Reset(); g_construction_mode = mode;
            if (create() != KEEL_RESULT_OK) return 1130;
            ...
            Identity()->m_pInstance = reinterpret_cast<CEntityInstance*>(allocated); instance = allocated;
            if (capture() != KEEL_RESULT_OK) { ::operator delete(allocated,std::align_val_t{16}); return 1131; }
