...
int RunEntityInputChecks()
{
    ...
    KeelCs2EntityInputBindings bindings{FunctionAddress(&InputEntityFixture),
                                        FunctionAddress(&QueueEntityInputFixture)};
    ...
    g_io_mode = g_io_calls = 0;
    g_io_source = nullptr;
    g_io_activator = g_controller_storage.data();
    g_io_caller = EntityInstance();
    void* value{};
    KeelBool invoked{};

    for (unsigned type = 0; type <= 8; ++type) {
        g_io_expected = {};
        g_io_expected.size = sizeof(g_io_expected);
        g_io_expected.type = type;
        g_io_expected.string_value = "input value";
        g_io_expected.int_value = 1;
        g_io_expected.float_value = 2.5f;
        g_io_expected.vector_value[0] = 5;
        g_io_expected.vector_value[1] = -90;
        g_io_expected.vector_value[2] = 17;
        g_io_expected.color_value[0] = 55;
        g_io_expected.color_value[3] = 255;
        g_io_expected.entity_handle = controller.source2_handle;

        if (KeelCs2Variant_Build(&g_io_expected, &value) != KEEL_RESULT_OK)
            return 1301;

        const auto* value_entity = type == 8 ? &controller : nullptr;
        char name[] = "Enable";
        g_io_source = name;
        auto result = KeelCs2_EntityInput(EntitySystem(),
                                          &target,
                                          &controller,
                                          &target,
                                          value_entity,
        ...
        if (result != KEEL_RESULT_OK || !invoked || !g_io_arguments || name[0] != 'X')
            return 1302;

        g_io_source = nullptr;
        const auto before = g_io_calls;
        result = KeelCs2_EntityInput(EntitySystem(),
                                     &target,
                                     &controller,
                                     &target,
                                     value_entity,
        ...
        if (type == 7) {
            if (result != KEEL_RESULT_UNSUPPORTED || invoked || g_io_calls != before || g_io_copy)
                return 1303;
        } else if (result != KEEL_RESULT_OK || !invoked || !g_io_arguments || !g_io_copy) return 1304;
        KeelCs2Variant_Release(value);

        if (g_io_copy) {
            if (KeelFixtureVariantInspect(g_io_copy, &g_io_expected))
                return 1305;

            KeelFixtureVariantRelease(g_io_copy);
            g_io_copy = nullptr;
        }
    ...
    if (call(&mismatch, nullptr, nullptr, nullptr, "Enable") != KEEL_RESULT_NOT_FOUND || invoked)
        return 1309;

    for (auto flag : {EF_IS_PRE_SPAWN,
                      EF_IS_CONSTRUCTION_IN_PROGRESS,
                      EF_MARKED_FOR_DELETE,
                      EF_DELETE_IN_PROGRESS,
                      EF_IS_INVALID_EHANDLE})
    {
        Identity()->m_flags = flag;

        if (call(&target, nullptr, nullptr, nullptr, "Enable") != KEEL_RESULT_NOT_FOUND || invoked)
            return 1310;
    }
    ...
    for (const char* name : {"","bad\nname",long_name.c_str(),static_cast<const char*>(nullptr)})
        if (call(&target, nullptr, nullptr, nullptr, name) != KEEL_RESULT_INVALID_ARGUMENT || invoked)
            return 1311;

    for (float delay : {-1.0f,std::numeric_limits<float>::infinity(),std::numeric_limits<float>::quiet_NaN()})
        if (call(&target, nullptr, nullptr, nullptr, "Enable", KEEL_TRUE, delay) != KEEL_RESULT_INVALID_ARGUMENT ||
            invoked)
            return 1312;
    ...
    if (call(&target, nullptr, nullptr, nullptr, "Enable", 2) != KEEL_RESULT_INVALID_ARGUMENT || invoked ||
        ...
        call(&target, nullptr, nullptr, &controller, "Enable") != KEEL_RESULT_INVALID_ARGUMENT || invoked ||
        g_io_calls != before)
        return 1313;

    g_io_activator = g_io_caller = nullptr;

    if (call(&target, nullptr, nullptr, nullptr, "Enable") != KEEL_RESULT_OK || !invoked || !g_io_arguments)
        return 1314;

    KeelCs2Variant_Release(value);
    g_io_expected.type = 8;
    g_io_expected.entity_handle = controller.source2_handle;
    ...
    if (call(&target, nullptr, nullptr, &controller, "Enable") != KEEL_RESULT_NOT_FOUND || invoked)
        return 1317;
    ...
    for (KeelBool queued : {KEEL_FALSE,KEEL_TRUE}) for (unsigned mode : {1u,2u}) {
            Reset();
            KeelCs2_FindEntityByIndex(EntitySystem(), kEntityIndex, &target);
            void* dynamic = ::operator new(kEntitySize, std::align_val_t{16});
            std::memcpy(dynamic, g_entity_storage.data(), kEntitySize);
            Identity()->m_pInstance = static_cast<CEntityInstance*>(dynamic);
            g_io_mode = mode;

            if (call(&target, nullptr, nullptr, nullptr, "Enable", queued, queued ? 1.25f : 0.0f) !=
                    (mode == 1 ? KEEL_RESULT_OK : KEEL_RESULT_ENGINE_FAILURE) ||
                !invoked || !g_io_arguments)
                return 1319;
...
public:
    ...
    KeelResult InputCapabilities(std::uint32_t& direct, std::uint32_t& queued) override {
        const auto callback = on_capabilities;
    ...
    KeelResult InputCurrent(std::uint64_t expected, void*& system, std::uint64_t& current) noexcept override {
        ++acquisitions;
        allocations_at_acquisition = KeelFixtureKeyValuesMemoryCount();
        system = nullptr;
        current = 0;
        ...
        system = null_system ? nullptr : EntitySystem();
        current = epoch;
...
int RunNativeInputChecks()
{
    ...
    keels2::cs2::NativeInputBackend backend(environment);
    KeelCs2EntityIdentity target{}, controller{};
    ...
    for (unsigned type = 0; type <= 8; ++type) {
        ...
        g_io_expected.int_value = 1;
        g_io_expected.float_value = -2.5f;
        g_io_expected.vector_value[1] = 17;
        g_io_expected.color_value[3] = 255;
        g_io_expected.entity_handle = controller.source2_handle;
        request.value.type = type;
        request.value.string_value = g_io_expected.string_value;
        request.value.int_value = g_io_expected.int_value;
        request.value.float_value = g_io_expected.float_value;
        ...
        if (call() != KEEL_RESULT_OK || !invoked || !g_io_arguments || KeelFixtureKeyValuesMemoryCount())
            return 1331;

        request.queued = KEEL_TRUE;
        request.delay = 1.25f;
        const auto before = g_io_calls;
        const auto result = call();

        if (type == KEELS2_INPUT_COLOR) {
            if (result != KEEL_RESULT_UNSUPPORTED || invoked || g_io_calls != before)
                return 1332;
        } else {
    ...
    g_io_expected.type = KEELS2_INPUT_STRING;
    g_io_expected.string_value = "copied";
    ...
    request.input = "Enable";
    request.value.type = KEELS2_INPUT_STRING;
    request.value.string_value = "copied";
    ...
    for (int after : {0,1}) for (bool throws : {false,true}) {
        ...
        if (!valid || before != environment.acquisitions)
            return 1336;
    }
    ...
    environment.on_capabilities = {};
    environment.stopping = false;
    environment.null_system = true;

    if (!reject(KEEL_RESULT_NOT_FOUND))
        return 1339;

    environment.null_system = false;
    environment.capabilities_result = KEEL_RESULT_UNSUPPORTED;

    if (!reject(KEEL_RESULT_UNSUPPORTED))
        return 1340;
    ...
    --request.activator.epoch;
    request.value_entity.index = 4;

    if (!reject(KEEL_RESULT_INVALID_ARGUMENT))
        return 1342;

    request.value_entity = {};
    environment.on_capabilities = [&]
    {
        Identity()->m_flags = EF_MARKED_FOR_DELETE;
    };
    ...
    if (call() != KEEL_RESULT_ENGINE_FAILURE || !invoked || !g_io_arguments || KeelFixtureKeyValuesMemoryCount())
        return 1344;
