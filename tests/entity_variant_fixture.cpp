...
{
    if (!pointer || !expected || !KeelFixtureKeyValuesMemoryOwns(pointer))
        return 1;
    ...
    if (value.m_flags != (owned ? CV_FREE : 0) || (owned && !KeelFixtureKeyValuesMemoryOwns(value.m_pData)))
        return 2;

    switch (e.type) {
        case KEELS2_CS2_VARIANT_VOID: return value.m_type == FIELD_VOID && value.m_pData == nullptr ? 0 : 3;
        case KEELS2_CS2_VARIANT_STRING:
            return value.m_type == FIELD_CSTRING && !std::strcmp(value.m_pszString, e.string_value) ? 0 : 4;
        ...
        case KEELS2_CS2_VARIANT_VECTOR:
            return value.m_type == FIELD_VECTOR &&
                           *value.m_pVector == Vector(e.vector_value[0], e.vector_value[1], e.vector_value[2])
                       ? 0
                       : 8;

        case KEELS2_CS2_VARIANT_ANGLES:
            return value.m_type == FIELD_QANGLE &&
                           *value.m_pQAngle == QAngle(e.vector_value[0], e.vector_value[1], e.vector_value[2])
                       ? 0
                       : 9;

        case KEELS2_CS2_VARIANT_COLOR:
            return value.m_type == FIELD_COLOR32 &&
                           *value.m_pColor ==
        ...
        case KEELS2_CS2_VARIANT_ENTITY:
            return value.m_type == FIELD_EHANDLE && static_cast<uint32_t>(value.m_hEntity.ToInt()) == e.entity_handle
                       ? 0
                       : 11;
...
extern "C" VARIANT_INSPECT_EXPORT void KeelFixtureVariantRelease(void* pointer)
{
    delete static_cast<CVariant*>(pointer);
}
