...
public:
    ...
    bool Load() override
    {
        ...
        if (!resolved || !HookPre(&g_deferred_target, &DeferredDispatch::Run, &SchemaEntityPlugin::BeforeDispatch) ||
            !CreateCommand("keel_schema_entity_check",
    ...
    {
        if (captured_ && captured_api_)
        {
            captured_api_->release(g_owner, captured_);
            captured_ = 0;
        }
...
private:
    keels2::kh::Action BeforeDispatch()
    {
    ...
    bool HookData()
    {
        if (!g_hook_record || !g_hook_component || !g_hook_set_callback || !g_hook_change_map)
            return false;
        ...
        if (g_api->query_service(g_owner, KEELS2_ENTITIES_SERVICE_NAME, 1, &raw) != KEEL_RESULT_OK || !raw)
            return false;

        const auto* entities = static_cast<const KeelEntitiesApi*>(raw);
        KeelEntityHandle pawn{};

        if (entities->find_by_index(g_owner, 7, &pawn) != KEEL_RESULT_OK)
            return false;
        ...
        auto bad = edit;
        bad.force[2] = std::numeric_limits<float>::quiet_NaN();
        ...
        if (g_hook_data->weapon_matches(g_owner, pawn, g_hook_component, &matches) != KEEL_RESULT_OK || !matches ||
            g_hook_data->weapon_matches(g_owner, pawn, reinterpret_cast<void*>(1), &matches) != KEEL_RESULT_OK ||
            matches ||
            g_hook_data->weapon_matches(g_owner, pawn + 10000, g_hook_component, &matches) != KEEL_RESULT_NOT_FOUND ||
            matches)
            return false;
        ...
        if (nested_result != KEEL_RESULT_OK || !g_hook_valid || g_hook_nested != 8)
            return false;
    ...
    bool Access()
    {
        ...
        if (access->size != sizeof(*access) || access->api_version != 1 || !access->visit)
            return false;
        ...
        if (g_api->query_service(g_owner, KEELS2_ENTITY_CAPTURE_SERVICE_NAME, 2, &capture_raw) !=
                KEEL_RESULT_INCOMPATIBLE ||
            g_api->query_service(g_owner, KEELS2_ENTITY_CAPTURE_SERVICE_NAME, 1, &capture_raw) != KEEL_RESULT_OK ||
            !capture_raw)
            return false;

        const auto* capture = static_cast<const KeelEntityCaptureApi*>(capture_raw);

        if (capture->size != sizeof(*capture) || capture->api_version != 1 || !capture->capture)
            return false;
        ...
        struct CaptureProbe
        {
            ...
            KeelEntityHandle source;
            KeelEntityHandle* retained;
        } capture_probe{capture, entities, entity, &captured_};
        const auto capture_pointer = [](void* data, void* const* pointers, std::uint32_t count) -> KeelResult {
            ...
            if (!pointers || count != 1 || !pointers[0])
        ...
        if (access->visit(g_owner, specs, 1, capture_pointer, &capture_probe) != KEEL_RESULT_OK)
        {
            entities->release(g_owner, entity);
            return false;
        }
    ...
    void OffThread()
    {
        ...
        {
            LogError("entity access wrong-thread lookup failed");
            return;
        }
        ...
        if (access->visit(
                ...
                &access_spec,
                1,
                [](void*, void* const*, std::uint32_t)
                {
                    return KEEL_RESULT_OK;
                },
                nullptr) != KEEL_RESULT_WRONG_THREAD)
        {
            LogError("entity access wrong-thread rejection failed");
            return;
        }

        const void* capture_raw{};

        if (g_api->query_service(g_owner, KEELS2_ENTITY_CAPTURE_SERVICE_NAME, 1, &capture_raw) != KEEL_RESULT_OK ||
            !capture_raw)
        {
            LogError("entity capture wrong-thread lookup failed");
            return;
        }
        ...
        {
            LogError("entity capture wrong-thread rejection failed");
            return;
        }
        ...
        {
            LogError("hook data wrong-thread lookup failed");
            return;
        }

        const auto* hook = static_cast<const KeelEntityHookDataApi*>(hook_raw);
        KeelDamageInfo damage{};
        ...
        {
            LogError("hook data wrong-thread rejection failed");
            return;
        }
        ...
        {
            LogError("entity write wrong-thread service lookup failed");
            return;
        }
        ...
        {
            LogError("entity tools wrong-thread lookup failed");
            return;
        }
        ...
        if (tools->capabilities(g_owner, &capabilities) != KEEL_RESULT_WRONG_THREAD || capabilities ||
            ...
            tools->set_model(g_owner, 1, "models/test.vmdl") != KEEL_RESULT_WRONG_THREAD ||
            tools->remove(g_owner, 1) != KEEL_RESULT_WRONG_THREAD)
        {
            LogError("entity tools wrong-thread rejection failed");
            return;
        }
        ...
        {
            LogError("entity write wrong-thread rejection failed");
            return;
        }

        if (g_api->query_service(g_owner, KEELS2_ROUND_CONTROL_SERVICE_NAME, 1, &raw) != KEEL_RESULT_OK || !raw)
        {
            LogError("round control wrong-thread service lookup failed");
            return;
        }
        ...
        {
            LogError("round control wrong-thread rejection failed");
            return;
        }

        if (g_api->query_service(g_owner,KEELS2_PLAYER_STATISTICS_SERVICE_NAME,1,&raw) != KEEL_RESULT_OK || !raw)
        {
            LogError("statistics wrong-thread lookup failed");
            return;
        }
        ...
        {
            LogError("statistics wrong-thread rejection failed");
            return;
        }
    ...
    bool Actions()
    {
        ...
        valid =
            valid &&
            g_api->query_service(g_owner, KEELS2_PLAYER_MANAGEMENT_SERVICE_NAME, 2, &management_raw) ==
                KEEL_RESULT_INCOMPATIBLE &&
        ...
        request.kind = 7;
        valid = valid && management->apply(g_owner, entity, &request) == KEEL_RESULT_INVALID_ARGUMENT;
        request.kind = KEELS2_PLAYER_MANAGEMENT_CHANGE_TEAM;
        request.team = 256;
        valid = valid && management->apply(g_owner, entity, &request) == KEEL_RESULT_INVALID_ARGUMENT;
        const void* stat_raw{};
        valid = valid &&
                g_api->query_service(g_owner, KEELS2_PLAYER_STATISTICS_SERVICE_NAME, 2, &stat_raw) ==
                    KEEL_RESULT_INCOMPATIBLE &&
        ...
        if (!valid || !stats || stats->size != sizeof(*stats) || stats->api_version != 1 || !stats->capabilities ||
            !stats->read || !stats->write)
            return false;

        std::uint32_t readable = UINT32_MAX, writable = UINT32_MAX;
        std::int32_t stat_value = 99;
        valid = valid && stats->capabilities(g_owner, &readable, &writable) == KEEL_RESULT_UNSUPPORTED && !readable &&
                !writable && stats->capabilities(g_owner, nullptr, &writable) == KEEL_RESULT_INVALID_ARGUMENT &&
                stats->read(g_owner, entity, KEELS2_PLAYER_STAT_MONEY, &stat_value) == KEEL_RESULT_UNSUPPORTED &&
                !stat_value && stats->write(g_owner, entity, KEELS2_PLAYER_STAT_MONEY, 1) == KEEL_RESULT_UNSUPPORTED &&
        ...
        valid = valid &&
                g_api->query_service(g_owner, KEELS2_ROUND_CONTROL_SERVICE_NAME, 2, &round_raw) ==
                    KEEL_RESULT_INCOMPATIBLE &&
        ...
        if (!valid || !round || round->size != sizeof(*round) || round->api_version != 1 || !round->capabilities ||
            !round->terminate)
            return false;
        ...
        termination.reserved = 1;
        valid = valid && round->terminate(g_owner, &termination) == KEEL_RESULT_INVALID_ARGUMENT;
        termination.reserved = 0;
        termination.delay = -1;
        valid = valid && round->terminate(g_owner, &termination) == KEEL_RESULT_INVALID_ARGUMENT;
        termination.delay = 1;
        --termination.size;
        ...
        if (g_api->query_service(g_owner, KEELS2_ENTITY_TOOLS_SERVICE_NAME, 2, &tools_raw) !=
                KEEL_RESULT_INCOMPATIBLE ||
            g_api->query_service(g_owner, KEELS2_ENTITY_TOOLS_SERVICE_NAME, 1, &tools_raw) != KEEL_RESULT_OK ||
            !tools_raw)
            return false;
        ...
        if (tools->size != sizeof(*tools) || tools->api_version != 1 || !tools->capabilities || !tools->teleport ||
            !tools->set_model || !tools->remove ||
            ...
            tools->remove(g_owner, UINT64_MAX) != KEEL_RESULT_NOT_FOUND ||
            tools->remove(g_owner, 0) != KEEL_RESULT_INVALID_ARGUMENT ||
            ...
            tools->set_model(g_owner, 1, "bad\nasset") != KEEL_RESULT_INVALID_ARGUMENT)
            return false;

        teleport.flags = 0;

        if (tools->teleport(g_owner, 1, &teleport) != KEEL_RESULT_INVALID_ARGUMENT)
            return false;

        const void* writes_raw{};
        valid = valid &&
                g_api->query_service(g_owner, KEELS2_ENTITY_WRITES_SERVICE_NAME, 2, &writes_raw) ==
                    KEEL_RESULT_INCOMPATIBLE &&
        ...
        if (!valid || !writes || writes->size != sizeof(*writes) || !writes->capabilities || !writes->write_field)
            return false;

        if (g_api->query_service(g_owner, KEELS2_SCHEMA_SERVICE_NAME, 1, &raw) != KEEL_RESULT_OK || !raw)
            return false;

        const auto* schema = static_cast<const KeelSchemaApi*>(raw);
        const KeelSchemaFieldSpec spec{
            sizeof(spec), KEELS2_SCHEMA_MODULE_SERVER, KEELS2_SCHEMA_INT32, 0, "CBaseEntity", "m_iHealth"};

        KeelSchemaFieldHandle field{};

        if (schema->resolve_field(g_owner, &spec, &field) != KEEL_RESULT_OK)
            return false;
...
{
    g_hook_record = record;
    g_hook_component = component;
    g_hook_set_callback = set_callback;
    g_hook_change_map = change_map;
}
