...
bool GetPlayer(void*, CPlayerSlot slot, google::protobuf::Message& info)
{
    ...
    if (player_mode == 2)
        throw std::runtime_error("player information failure");
...
{
    if (type != reinterpret_cast<INetworkMessageInternal*>(&definition) ||
        value != reinterpret_cast<CNetMessage*>(&message))
    {
...
int main()
{
    try
    {
        ...
        Check((recipients[last / 64] & (uint64{1} << (last % 64))) != 0 && (recipients[0] & 8),
              "recipient mask truncated");
