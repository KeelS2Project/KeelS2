...
namespace {
void Check(bool value, int line) { if (!value) throw std::runtime_error("host construction check "+std::to_string(line)); }
...
public:
    ...
    struct Record { GameEntityIdentity identity; bool live{}, closed{}, busy{}; };
    ...
    KeelResult FindEntityBySource2Handle(std::uint32_t source, GameEntityIdentity& out, std::string& error) override {
        out = {}; const auto it = records.find(source);
        if (it == records.end() || ValidateEntity(it->second->identity,error) != KEEL_RESULT_OK) return KEEL_RESULT_NOT_FOUND;
        out = it->second->identity; return KEEL_RESULT_OK;
    }
    static Adapter& Get(GameAdapter* adapter) { return *static_cast<Adapter*>(adapter); }
    static void Call(const std::function<void()>& function) { const auto copy = function; if (copy) copy(); }
    static GameAdapterEntityConstructionApi Api() {
        return {sizeof(GameAdapterEntityConstructionApi),1,
            [](GameAdapter*) noexcept -> KeelResult { return KEEL_RESULT_OK; },
            [](GameAdapter* adapter, const char*, std::uint64_t* token, GameEntityIdentity* identity) noexcept -> KeelResult {
                auto& a = Get(adapter); ++a.creates; *token = a.next++;
                *identity = {static_cast<int>(*token),static_cast<std::uint32_t>(*token),a.epoch};
                a.records.emplace(*token,std::make_shared<Record>(Record{*identity})); Call(a.on_create); return KEEL_RESULT_OK;
            },
            [](GameAdapter* adapter, std::uint64_t token, GameEntityIdentity* identity) noexcept -> KeelResult {
                *identity = {}; auto& a = Get(adapter); const auto it = a.records.find(token);
                if (it == a.records.end() || it->second->closed || it->second->live || it->second->identity.epoch != a.epoch) return KEEL_RESULT_NOT_FOUND;
                *identity = it->second->identity; return KEEL_RESULT_OK;
            },
            [](GameAdapter* adapter, std::uint64_t, const KeelEntityKeyValue* value) noexcept -> KeelResult {
                auto& a = Get(adapter); ++a.sets; const auto copied_name = std::string(value->name);
                const auto copied_text = std::string(value->string_value); Call(a.on_set);
                CHECK(copied_name == value->name && copied_text == value->string_value); return KEEL_RESULT_OK;
            },
            [](GameAdapter* adapter, std::uint64_t, const KeelEntityTeleport* request) noexcept -> KeelResult {
                auto& a = Get(adapter); ++a.teleports; const auto x = request->position[0]; Call(a.on_teleport);
                CHECK(x == request->position[0]); return KEEL_RESULT_OK;
            },
            [](GameAdapter* adapter, std::uint64_t token, KeelBool* invoked) noexcept -> KeelResult {
                auto& a = Get(adapter); *invoked = a.dispatch ? KEEL_TRUE : KEEL_FALSE;
                if (!a.dispatch) return a.spawn_result;
                auto record = a.records.at(token); record->busy = true; ++a.spawns; Call(a.on_spawn);
                record->busy = false; record->live = a.spawn_result == KEEL_RESULT_OK;
                if (!record->live) a.records.erase(token);
                return a.spawn_result;
            },
            [](GameAdapter* adapter, std::uint64_t token) noexcept -> KeelResult {
                auto& a = Get(adapter); const auto it = a.records.find(token);
                if (it == a.records.end()) return KEEL_RESULT_NOT_FOUND;
                auto record = it->second; record->closed = true; ++a.cancels;
                if (!record->busy && !record->live) a.records.erase(it);
                Call(a.on_cancel); return KEEL_RESULT_OK;
            },
            [](GameAdapter* adapter, std::uint64_t token, const char* name, KeelEntityAccessCallback callback, void* data) noexcept -> KeelResult {
                auto& a = Get(adapter); const auto it = a.records.find(token);
                if (it == a.records.end() || it->second->closed) return KEEL_RESULT_NOT_FOUND;
                if (std::string(name) != "CBaseEntity") return KEEL_RESULT_INCOMPATIBLE;
                auto hold = it->second; void* pointer = hold.get(); return callback(data,&pointer,1);
            }};
    }
};
KeelResult Visit(void* data, void* const* pointers, std::uint32_t count)
{
    CHECK(count == 1 && pointers[0]); (*static_cast<std::function<void()>*>(data))(); return KEEL_RESULT_OK;
}
}
struct SchemaEntityServiceTest
{
    static void Run()
    {
        ...
        host.accepting_resources_ = true; host.adapter_ = &adapter;
        ...
        for (unsigned i : {1u,2u}) {
            auto record = std::make_unique<PluginRecord>(); record->handle = i;
            record->state = PluginState::loaded; record->accepting_resources = true;
        ...
        const auto create = [&] { CHECK(api.create(1,"prop_dynamic",&owner) == KEEL_RESULT_OK && owner); };
        const auto observe = [&] { CHECK(api.describe(1,owner,&info) == KEEL_RESULT_OK);
        ...
        create(); observe(); const auto source = info.source2_handle;
        ...
        KeelEntityKeyValue value{}; value.size = sizeof(value); value.type = KEELS2_ENTITY_KEY_STRING;
        char name[] = "model", text[] = "models/test.vmdl"; value.name = name; value.string_value = text;
        CHECK(api.set(2,observer,&value) == KEEL_RESULT_NOT_FOUND);
        CHECK(api.set(2,owner,&value) == KEEL_RESULT_NOT_FOUND);
        adapter.on_set = [&] { name[0] = text[0] = 'X'; };
        CHECK(api.set(1,owner,&value) == KEEL_RESULT_OK && adapter.sets == 1); adapter.on_set = {};
        auto invalid = value; invalid.type = KEELS2_ENTITY_KEY_FLOAT; invalid.float_value = std::numeric_limits<float>::infinity();
        ...
        adapter.on_teleport = [&] { teleport.position[0] = 99; };
        CHECK(api.teleport(1,owner,&teleport) == KEEL_RESULT_OK); adapter.on_teleport = {};
        std::thread worker([&] {
        ...
        CHECK(!adapter.cancels); observe();
        adapter.on_cancel = [&] {
            CHECK(host.PluginByHandle(1)->active_native_operations);
            std::string reason; CHECK(host.HasRunningDependent(*host.PluginByHandle(1),reason,true));
        ...
        CHECK(entities.release(1,owner) == KEEL_RESULT_OK); adapter.spawn_result = KEEL_RESULT_OK;
        // Factory callbacks can revoke the reserved host handle before return.
        create(); const auto guessed = owner+1; CHECK(entities.release(1,owner) == KEEL_RESULT_OK);
        adapter.on_create = [&] { CHECK(entities.release(1,guessed) == KEEL_RESULT_OK); };
        ...
        for (unsigned i = 0; i < 64; ++i) { create(); pending.push_back(owner); }
        CHECK(api.create(1,"prop_dynamic",&owner) == KEEL_RESULT_BUSY && !owner);
        ++adapter.epoch; const auto before_prune = adapter.cancels;
        create(); CHECK(adapter.cancels == before_prune+64);
        for (const auto handle : pending) CHECK(entities.release(1,handle) == KEEL_RESULT_OK);
        std::vector<KeelEntityHandle> observers;
        for (unsigned i = 0; i < 256; ++i) { observe(); observers.push_back(observer); }
        CHECK(api.observe(2,info.source2_handle,&observer) == KEEL_RESULT_BUSY && !observer);
        CHECK(service.ReleasePlugin(2) == KEEL_RESULT_OK);
        CHECK(api.describe(1,owner,&info) == KEEL_RESULT_OK); observe();
        ...
        create(); observe(); const auto shutdown_observer = observer;
        create(); host.state_ = HostState::running; host.dispatch_open_.store(true);
        CHECK(BeginGameCommandDispatch() == 1);
        unsigned shutdown_callbacks{};
        adapter.on_cancel = [&] { ++shutdown_callbacks; CHECK(!host.Stop()); };
        CHECK(entities.release(1,owner) == KEEL_RESULT_OK);
        CHECK(host.state_ == HostState::running && host.schema_entities_);
        EndGameCommandDispatch();
        CHECK(host.Stop()); adapter.on_cancel = {};
