...
struct OutputHookProbe final : keels2::cs2::OutputEnvironment {
    ...
    OutputHookProbe() {
        active = this;
        api.size = sizeof(api);
        api.api_version = KEELHOOK_API_VERSION;
        api.resolve_target = &Resolve;
        api.add_callback = &Add;
        api.remove_callback = &Remove;
        api.release_target = &Release;
    }
    ...
    static KeelResult Resolve(KeelPluginHandle owner, const KeelHookTargetSpec* spec,
        ...
        for (unsigned i = 0; i < 7; ++i)
            p.Require(prototype->argument_types[i] == (i == 4 ? KH_VALUE_FLOAT32 : KH_VALUE_POINTER));
    ...
    {
        auto& p = *active;
        p.Require(!owner && target == 77 && spec->phases == KH_PHASE_BOTH);

        if (p.add_result != KEEL_RESULT_OK)
            return p.add_result;
    ...
    static KeelResult Remove(KeelPluginHandle owner, KeelHookCallbackHandle handle) {
        auto& p = *active;
        ++p.removes;
        p.Require(!owner && handle == 88);

        if (p.remove_result == KEEL_RESULT_OK)
            p.installed = {};
    ...
    static KeelResult Release(KeelPluginHandle owner, KeelHookTargetHandle target) {
        auto& p = *active;
        ++p.releases;
        p.Require(!owner && target == 77);
        return p.release_result;
    }

    static KeelResult Defer(KeelHookFrame*, void (*cleanup)(void*), void* data) noexcept {
        if (active->defer_result != KEEL_RESULT_OK)
            return active->defer_result;

        try
        {
            active->cleanups.emplace_back(cleanup, data);
            return KEEL_RESULT_OK;
        }
        catch (...)
        {
    ...
    static std::uint32_t Notify(const KeelEntityOutputEvent* event, std::uint64_t token, void* data) {
        ...
        if (!event) {
            ...
            p.Require(found != p.tokens.end());

            if (found != p.tokens.end())
                p.tokens.erase(found);
        ...
        if (event->phase == KEELS2_OUTPUT_PRE) {
            ++p.pre;
            p.Require(!event->flags && std::find(p.tokens.begin(), p.tokens.end(), token) == p.tokens.end());
            ...
            if (p.throwing)
                throw std::runtime_error("output observer failure");

            return p.block ? KEELS2_OUTPUT_BLOCK : KEELS2_OUTPUT_CONTINUE;
        }

        ++p.post;
        p.Require(event->phase == KEELS2_OUTPUT_POST &&
                  std::find(p.tokens.begin(), p.tokens.end(), token) != p.tokens.end());
    ...
    KeelResult Start()
    {
        return hooks.Start(FunctionAddress(&InputEntityFixture), "fixture", api, &Defer, &Notify, this);
    }

    void Fire(float delay = 1.25f) {
        const auto end = cleanups.size();
        const auto callback = installed;
        std::array<KeelHookValue,7> args{};

        for (auto& arg : args)
            arg.type = KH_VALUE_POINTER;

        args[0].scalar.pointer = output;
        args[2].scalar.pointer = g_controller_storage.data();
        args[3].scalar.pointer = value;
        args[4].type = KH_VALUE_FLOAT32;
        args[4].scalar.float32 = delay;
        args[5].scalar.pointer = reinterpret_cast<void*>(5);
        ...
        if (!skip_post)
            Require(callback.callback(&frame, callback.user_data) == KH_ACTION_CONTINUE);

        while (cleanups.size() != end) {
            const auto entry = cleanups.back();
            cleanups.pop_back();
...
int RunOutputHookChecks()
{
    ...
    } output{nullptr, nullptr, &descriptor};
    auto probe = std::make_unique<OutputHookProbe>();
    auto& p = *probe;
    const auto reset = [&]
    {
        InputFixture(0);
        ...
        p.output = g_entity_storage.data() + offset;
        p.pre = p.post = p.completed = p.original = 0;
        p.before = {};
        p.engine = {};
        p.block = p.skip_post = p.throwing = false;
    };
    reset();
    KeelCs2VariantValue value{};
    value.size = sizeof(value);
    value.type = KEELS2_INPUT_STRING;
    value.string_value = "copied output";

    if (KeelCs2Variant_Build(&value, &p.value) != KEEL_RESULT_OK)
        return 1500;

    if (p.Start() != KEEL_RESULT_OK || !p.hooks.Started() || p.Start() != KEEL_RESULT_OK || p.resolves != 1)
        return 1501;

    if (p.hooks.Start(FunctionAddress(&QueueEntityInputFixture),
                      "fixture",
                      p.api,
                      &OutputHookProbe::Defer,
                      &OutputHookProbe::Notify,
                      &p) != KEEL_RESULT_BUSY)
        return 1519;
    ...
    if (p.pre != 1 || p.post != 1 || p.completed != 1 || p.original != 2 || !p.tokens.empty())
        return 1520;
    ...
    if (p.pre != 1 || p.post != 1 || p.completed != 1 || p.original || p.observed.flags)
        return 1503;
    ...
    if (p.pre != 1 || p.post || p.completed != 1 || !p.tokens.empty())
        return 1505;
    ...
    if (p.original || p.observed.flags || p.completed != 1)
        return 1506;
    ...
    p.engine = [&]
    {
        KeelCs2Variant_Release(p.value);
        p.value = nullptr;
    ...
    if (p.pre != 1 || p.observed.value_status != KEEL_RESULT_INVALID_ARGUMENT || p.observed.value.type != UINT32_MAX)
        return 1509;
    ...
    if (p.pre != 64 || p.post != 64 || p.completed != 64 || p.original != 65 || !p.tokens.empty())
        return 1510;

    reset();
    p.defer_result = KEEL_RESULT_BUSY;
    p.Fire();
    p.defer_result = KEEL_RESULT_OK;

    if (p.pre || p.post || p.completed || p.original != 1)
        return 1511;

    reset();
    p.readiness = KEEL_RESULT_NOT_READY;
    p.Fire();
    p.readiness = KEEL_RESULT_OK;

    if (p.pre || p.completed || p.original != 1)
        return 1512;

    reset();
    p.Fire(std::numeric_limits<float>::infinity());
    ...
    if (p.hooks.Stop() != KEEL_RESULT_BUSY || !p.hooks.Started())
        return 1514;

    p.remove_result = KEEL_RESULT_OK;
    p.release_result = KEEL_RESULT_BUSY;

    if (p.hooks.Stop() != KEEL_RESULT_BUSY || p.hooks.Started() || p.Start() != KEEL_RESULT_BUSY)
        return 1515;

    p.release_result = KEEL_RESULT_OK;

    if (p.hooks.Stop() != KEEL_RESULT_OK || p.Start() != KEEL_RESULT_OK || p.hooks.Stop() != KEEL_RESULT_OK)
        return 1516;

    p.add_result = KEEL_RESULT_ENGINE_FAILURE;
    p.release_result = KEEL_RESULT_BUSY;

    if (p.Start() != KEEL_RESULT_ENGINE_FAILURE || p.hooks.Started() || p.Start() != KEEL_RESULT_BUSY)
        return 1517;

    p.release_result = KEEL_RESULT_OK;

    if (p.hooks.Stop() != KEEL_RESULT_OK || p.failed || !p.tokens.empty() || KeelFixtureKeyValuesMemoryCount())
        return 1518;

    KeelFixtureKeyValuesMemoryStop();
    g_input_handle.Get()->m_sTypeName.Purge();
    Reset();
    OutputHookProbe::active = nullptr;
    return 0;
}
