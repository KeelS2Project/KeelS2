...
int main()
{
    KeelCs2VariantSnapshot snapshot{}; snapshot.size = sizeof(snapshot);
    const auto read = [&](const CVariant& value) { return KeelCs2Variant_Read(&value,&snapshot); };
    const auto cleared = [&] {
        KeelCs2VariantSnapshot expected{};
        expected.size = sizeof(expected); expected.type = UINT32_MAX; expected.native_type = snapshot.native_type;
    ...
    for (bool flag : {false,true}) {
        CVariant value(flag);
        CHECK(read(value) == KEEL_RESULT_OK && snapshot.type == KEELS2_CS2_VARIANT_BOOL && snapshot.int_value == static_cast<int32_t>(flag));
    }
    ...
    char text[] = "output \xC3\xA9";
    {
        ...
        CHECK(read(value) == KEEL_RESULT_OK && snapshot.type == KEELS2_CS2_VARIANT_STRING && !std::strcmp(snapshot.string_value,text));
    ...
    for (const char* string : {static_cast<const char*>(nullptr),"","pooled output"}) {
        ...
        CHECK(read(value) == KEEL_RESULT_OK && snapshot.native_type == FIELD_STRING && !std::strcmp(snapshot.string_value,string ? string : ""));
    }
    CHECK(!std::strcmp(retained.string_value,"output \xC3\xA9"));
    char bounded[KEELS2_CS2_VARIANT_MAX_STRING+2]; std::memset(bounded,'s',sizeof(bounded));
    ...
    bounded[KEELS2_CS2_VARIANT_MAX_STRING] = 's'; bounded[KEELS2_CS2_VARIANT_MAX_STRING+1] = 0;
    CHECK(read(string) == KEEL_RESULT_INCOMPATIBLE && cleared());
    {
        Vector vector(1.25f,-8,90); CVariant value; value = &vector;
    ...
    {
        QAngle angles(-90,45.5f,180); CVariant value; value = &angles;
    ...
    {
        Color color(3,200,255,0); CVariant value; value = &color;
        CHECK(read(value) == KEEL_RESULT_OK && snapshot.type == KEELS2_CS2_VARIANT_COLOR);
        color.SetColor(9,9,9,9);
    }
    CHECK(snapshot.color_value[0] == 3 && snapshot.color_value[1] == 200 && snapshot.color_value[2] == 255 && !snapshot.color_value[3]);
    for (float invalid : {std::numeric_limits<float>::infinity(),-std::numeric_limits<float>::infinity(),std::numeric_limits<float>::quiet_NaN()}) {
        CVariant value(invalid); CHECK(read(value) == KEEL_RESULT_INCOMPATIBLE && cleared());
        for (int axis = 0; axis < 3; ++axis) {
            Vector vector(1,2,3); vector[axis] = invalid; CVariant v; v = &vector;
            CHECK(read(v) == KEEL_RESULT_INCOMPATIBLE && cleared());
            QAngle angles(1,2,3); angles[axis] = invalid; CVariant a; a = &angles;
