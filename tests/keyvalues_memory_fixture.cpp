...
{
    if (!value)
        return size ? Allocate(size, alignment) : nullptr;
    ...
    std::memcpy(result, value, std::min(size, old_size));
    Free(value);
    return result;
}

void* AllocSlot(void*, std::size_t size)
{
    return Allocate(size);
}

void* ReallocSlot(void*, void* value, std::size_t size)
{
    return Reallocate(value, size, 16);
}

void FreeSlot(void*, void* value)
{
    Free(value);
}

void* AlignedSlot(void*, std::size_t size, std::size_t alignment)
{
    return Allocate(size, alignment);
}

void* ReallocAlignedSlot(void*, void* value, std::size_t size, std::size_t alignment)
{
    return Reallocate(value, size, alignment);
}

void* RegionSlot(void*, unsigned char, std::size_t size)
{
    return Allocate(size);
}

void RegionFreeSlot(void*, unsigned char, void* value)
{
    Free(value);
}

void* RegionAlignedSlot(void*, unsigned char, std::size_t size, std::size_t alignment)
{
    return Allocate(size, alignment);
}
...
{
    if (g_pMemAlloc || !allocations.empty())
        Unsupported();
    ...
    methods[first + 12] = Address(&RegionSlot);
    methods[first + 13] = methods[first + 15] = Address(&RegionFreeSlot);
...
extern "C" KV_FIXTURE_EXPORT std::size_t KeelFixtureKeyValuesMemoryCount()
{
    return allocations.size();
}

extern "C" KV_FIXTURE_EXPORT bool KeelFixtureKeyValuesMemoryOwns(void* value)
{
    return allocations.contains(value);
}

extern "C" KV_FIXTURE_EXPORT void KeelFixtureKeyValuesMemoryFailAfter(int count, bool throws)
{
    fail_after = count;
    throw_failure = throws;
}
...
{
    return g_pMemAlloc == reinterpret_cast<IMemAlloc*>(&memory);
}

extern "C" {
KV_FIXTURE_EXPORT int LOG_GENERAL = 0;

KV_FIXTURE_EXPORT bool LoggingSystem_IsChannelEnabled(int, int)
{
    return false;
}

KV_FIXTURE_EXPORT int LoggingSystem_Log(int, int, const char*, ...)
{
    Unsupported();
}

KV_FIXTURE_EXPORT void* V_tier0_memmove(void* dest, const void* source, std::size_t count)
{
    return std::memmove(dest, source, count);
}
...
KV_FIXTURE_EXPORT bool V_CompareNameWithWildcards(const char*, const char*)
{
    Unsupported();
}

KV_FIXTURE_EXPORT FixtureThreadId ThreadGetCurrentId()
{
    return 1;
}

KV_FIXTURE_EXPORT void Plat_NonFatalErrorFunc(const char*, ...)
{
    Unsupported();
}
}
...
{
public:
    KV_FIXTURE_EXPORT void AcquireLock(FixtureThreadId);
};

void CAtomicMutex::AcquireLock(FixtureThreadId)
{
    Unsupported();
}

KV_FIXTURE_EXPORT void ThreadAtomicNotifyOne(const unsigned int*)
{
    Unsupported();
}
...
class CSplitString
{
    KV_FIXTURE_EXPORT void Split(const char*, int, const char**, int, bool);
};

void CSplitString::Split(const char*, int, const char**, int, bool)
{
    Unsupported();
}
