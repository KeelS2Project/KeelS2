...
public:
    ...
    bool Load() override
    {
...
#if defined(_WIN32)
            if (_putenv_s("KEELS2_TEST_PAUSED_REPLACEMENT", "") != 0) return false;
#else
            if (unsetenv("KEELS2_TEST_PAUSED_REPLACEMENT") != 0) return false;
#endif
        }
        const void* service{};
        if (!HostContext().PluginHandle() || HostContext().QueryService("keels2.unload", 1, &service) != KEEL_RESULT_OK || !service)
            return false;
        ...
        if (HostContext().QueryService(KEELS2_PAUSE_SERVICE_NAME, 999, &incompatible) != KEEL_RESULT_INCOMPATIBLE || incompatible)
            return false;
        const auto spec = Definition();
        if (!convars_ || convars_->create(HostContext().PluginHandle(), &spec, &variable_) != KEEL_RESULT_OK)
            return false;
        return CheckGameThread() == KEEL_RESULT_OK && CreateCommand("cleanup_test", "Controls cleanup fixture", &CleanupPlugin::Command);
    }
    bool PreparePause() override
    {
        ++pauses;
        if (CheckGameThread() != KEEL_RESULT_OK) return false;
        if (pause_mode == 2) throw std::runtime_error("pause fixture failure");
        if (pause_mode != 1) return false;
        const auto owner = HostContext().PluginHandle();
        if (pause_->set_prepare_callback(owner, nullptr, nullptr) != KEEL_RESULT_BUSY) return false;
        auto spec = Definition();
        spec.name = "keels2_pause_new";
        KeelConVarHandle rejected{};
        if (convars_->create(owner, &spec, &rejected) != KEEL_RESULT_NOT_READY || rejected) return false;
        if (variable_ && convars_->release(owner, variable_) != KEEL_RESULT_OK) return false;
        variable_ = 0;
    ...
    {
        if (plugin.id != HostContext().PluginHandle()) return;
        const auto spec = Definition();
...
extern "C" KEELS2_PLUGIN_EXPORT void KeelTest_PauseMode(unsigned value) { pause_mode = value; }
extern "C" KEELS2_PLUGIN_EXPORT unsigned KeelTest_PauseCalls() { return pauses; }
