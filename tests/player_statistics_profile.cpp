...
ModuleLookup FindPrimaryVtable(const LoadedModule&, std::string_view name, std::size_t count, void**& output, std::string&) {
    output = nullptr;
    if (name == failed_class) return ModuleLookup::ambiguous;
    if (lookup != ModuleLookup::found) return lookup;
    if (name == "CBaseEntity" && count == 30) output = table.data();
    if (name == "CCSPlayerController" && count == 30) output = controller.data();
    if (name == "CCSPlayerController_InGameMoneyServices" && count == 3) output = money.data();
    if (name == "CCSPlayerController_ActionTrackingServices" && count == 3) output = tracking.data();
...
int main() {
    ...
    std::string error;
#if defined(_WIN32)
    return cs2::ResolvePlayerStatistics(module, "cs2-25218825-linuxsteamrt64-40575640-b2ce91a0f330222a", bindings, error) == KEEL_RESULT_UNSUPPORTED ? 0 : 1;
#else
    ...
    module.ranges.push_back({image.data() + rva,bytes.size(),true,true}); table[29] = image.data() + rva; controller[29] = table[29];
    const auto resolve = [&](KeelResult expected, const char* profile = "cs2-25218825-linuxsteamrt64-40575640-b2ce91a0f330222a") {
        ...
        return result == expected && (result == KEEL_RESULT_OK ? bindings.notify == image.data()+rva &&
            bindings.controller_vtable == controller.data() && bindings.money_vtable == money.data() && bindings.tracking_vtable == tracking.data() :
            !bindings.notify && !bindings.controller_vtable && !bindings.money_vtable && !bindings.tracking_vtable);
    };
    ...
    table[29] = image.data(); if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 8; table[29] = image.data() + rva; controller[29] = table[29];
    lookup = platform::ModuleLookup::ambiguous; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 9; lookup = platform::ModuleLookup::found;
    controller[29] = image.data(); if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 11; controller[29] = table[29];
    for (const char* name : {"CCSPlayerController","CCSPlayerController_InGameMoneyServices","CCSPlayerController_ActionTrackingServices"}) {
        failed_class = name; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 12;
    }
    failed_class.clear();
