...
int main() {
    ...
    module.ranges.push_back({image.data() + rva,bytes.size(),true,true}); table[notify_slot] = image.data() + rva; controller[notify_slot] = table[notify_slot];
    ...
    table[notify_slot] = image.data(); if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 8; table[notify_slot] = image.data() + rva; controller[notify_slot] = table[notify_slot];
    lookup = platform::ModuleLookup::ambiguous; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 9; lookup = platform::ModuleLookup::found;
    controller[notify_slot] = image.data(); if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 11; controller[notify_slot] = table[notify_slot];
