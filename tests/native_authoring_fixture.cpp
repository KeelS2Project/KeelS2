...
bool Check(const KeelHookApi& api, KeelPluginHandle plugin)
{
    ...
    ConCommandRef reference(uint16(0x1234), 0x76543210);
    std::memset(reinterpret_cast<unsigned char*>(&reference) + sizeof(uint16), 0xa5, 2);
    KeelHookValue value{};
    ...
    if (!Adapter::Write(value, negative) || value.reserved != 0 ||
        ...
        Adapter::Read(value).GetRegisteredIndex() != -12345)
    {
        return false;
    }
