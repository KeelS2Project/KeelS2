...
bool PlayerServiceContract::Check(keels2::Context& context, const char* stage)
{
    ...
    if (std::strcmp(stage, "late") == 0)
    {
        ...
        if (context.QueryService(KEELS2_PLAYER_INPUT_SERVICE_NAME, 2, &service) != KEEL_RESULT_INCOMPATIBLE || service ||
            context.QueryService(KEELS2_PLAYER_INPUT_SERVICE_NAME, 1, &service) != KEEL_RESULT_OK || !service) return false;
        input_ = static_cast<const KeelPlayerInputApi*>(service);
        if (input_->size != sizeof(*input_) || input_->api_version != 1 || !input_->read) return false;
    ...
    if (std::strncmp(stage, "input", 5) == 0)
    {
        const auto connection = Connection(first_);
        KeelPlayerInput input{sizeof(input), 0, 0, 0};
        const auto empty = [&] { return input.size == sizeof(input) && !input.reserved && !input.buttons && !input.context; };
        if (std::strcmp(stage, "input-unavailable") == 0)
        {
            input.buttons = input.context = UINT64_MAX;
            return input_->read(plugin, &connection, &input) == KEEL_RESULT_NOT_READY && empty();
        }
        if (std::strcmp(stage, "input-replaced") == 0)
        {
            return input_->read(plugin, &connection, &input) == KEEL_RESULT_NOT_FOUND && empty() &&
                api_->get_player(plugin, 3, &player) == KEEL_RESULT_OK &&
                player.connection != connection.generation && player.user_id == 4302;
        }
        if (input_->read(plugin, &connection, &input) != KEEL_RESULT_OK || input.reserved ||
            input.buttons != (KEELS2_BUTTON_FORWARD | KEELS2_BUTTON_USE) || !input.context) return false;
        if (input_context_ && input.context == input_context_) return false;
        input_context_ = input.context;
        if (input_->read(plugin, &connection, &input) != KEEL_RESULT_OK || input.context != input_context_) return false;
        auto stale = connection; ++stale.generation;
        if (input_->read(plugin, &stale, &input) != KEEL_RESULT_NOT_FOUND || !empty()) return false;
        if (input_->read(0, &connection, &input) != KEEL_RESULT_NOT_READY || !empty()) return false;
        if (input_->read(plugin, nullptr, &input) != KEEL_RESULT_INVALID_ARGUMENT || !empty()) return false;
        input.size = 0;
        if (input_->read(plugin, &connection, &input) != KEEL_RESULT_INVALID_ARGUMENT || !empty()) return false;
        KeelResult wrong_thread{};
        std::thread worker([&] { wrong_thread = input_->read(plugin, &connection, &input); });
        worker.join();
        return wrong_thread == KEEL_RESULT_WRONG_THREAD && empty();
    }
