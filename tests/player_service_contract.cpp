...
bool PlayerServiceContract::Check(keels2::Context& context, const char* stage)
{
    ...
    if (std::strcmp(stage, "late") == 0)
    {
        ...
        if (input_->size != sizeof(*input_) || input_->api_version != 1 || !input_->read)
            return false;
    ...
    if (std::strncmp(stage, "input", 5) == 0)
    {
        ...
        const auto empty = [&]
        {
            return input.size == sizeof(input) && !input.reserved && !input.buttons && !input.context;
        };
        ...
        if (input_context_ && input.context == input_context_)
            return false;

        input_context_ = input.context;

        if (input_->read(plugin, &connection, &input) != KEEL_RESULT_OK || input.context != input_context_)
            return false;

        auto stale = connection;
        ++stale.generation;

        if (input_->read(plugin, &stale, &input) != KEEL_RESULT_NOT_FOUND || !empty())
            return false;

        if (input_->read(0, &connection, &input) != KEEL_RESULT_NOT_READY || !empty())
            return false;

        if (input_->read(plugin, nullptr, &input) != KEEL_RESULT_INVALID_ARGUMENT || !empty())
            return false;

        input.size = 0;

        if (input_->read(plugin, &connection, &input) != KEEL_RESULT_INVALID_ARGUMENT || !empty())
            return false;
        ...
        std::thread worker(
            [&]
            {
                wrong_thread = input_->read(plugin, &connection, &input);
            });
...
bool PlayerServiceContract::Unloaded(KeelPluginHandle plugin) const
{
    ...
    std::thread worker(
        [&]
        {
            result = api_->get_player(plugin, 3, &player);
        });
