...
public:
    ...
    bool Load() override
    {
        ...
        if (!Check(convars->create(Owner(), &count, &count_handle), "Create count")) return false;

        const KeelConVarSpec text = Spec("keel_docs_abi_text", Text("Ready"));
        if (!Check(convars->create(Owner(), &text, &text_handle), "Create text")) return false;

        const KeelConVarSpec native_count = Spec("keel_docs_abi_native", Integer(0));
        void* borrowed_native{};
        if (!Check(native->create_convar(Owner(), &native_count,
            [](void*, int32_t slot, const void* next, const void* previous, void* state) {
                if (!next || !previous) return;
                auto& self = *static_cast<ConVarAbi*>(state);
                self.LogMessage("Native count [{}]: {} -> {}", slot,
                    *static_cast<const int32_t*>(previous), *static_cast<const int32_t*>(next));
            }, this, &native_handle, &borrowed_native), "Create native count")) return false;

        if (!Check(convars->find(Owner(), count.name, KEELS2_CONVAR_INT32, &watched_handle), "Find count"))
            return false;
        if (!Check(observers->observe(Owner(), watched_handle,
            [](const KeelConVarChange* change, void* state) {
                if (!change || change->size != sizeof(*change)) return;
                auto& self = *static_cast<ConVarAbi*>(state);
                self.LogMessage("Observed {} on handle {} at slot {}", change->name, change->convar, change->slot);
            }, this), "Observe count")) return false;
...
private:
    ...
    {
        if (result != KEEL_RESULT_OK) LogError("{}: result {}", operation, result);
    ...
    void Show()
    {
        ...
        if (!Check(convars->describe(Owner(), count_handle, &info), "Describe count")) return;
    ...
    void Inspect()
    {
        Check(access->invoke(Owner(), count_handle, [](const void* reference, void* state) -> KeelResult {
            if (!reference || !g_pCVar) return KEEL_RESULT_NOT_READY;
            ConVarRef handle;
            std::memcpy(&handle, reference, sizeof(handle));
            CConVarRef<int32_t> value(handle);
            if (!value.IsValidRef() || !value.IsConVarDataAvailable() ||
                value.GetType() != TranslateConVarType<int32_t>()) return KEEL_RESULT_INCOMPATIBLE;
    ...
    void Retire()
    {
        for (KeelConVarHandle* handle : {&watched_handle, &native_handle, &text_handle, &count_handle})
        {
            ...
            if (result == KEEL_RESULT_OK || result == KEEL_RESULT_NOT_FOUND) *handle = 0;
