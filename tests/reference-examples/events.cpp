...
public:
    ...
    int32 CallbackPriority() const noexcept override
    {
        return 10;
    }
    ...
    {
        LogMessage("Initial plugin loading completed.");
    }

    bool OnClientConnect(CPlayerSlot slot, const char*, uint64, const char*, bool, CBufferString*) override
    {
        LogMessage("Connection proposed for slot {}", slot);
        return true;
    }

    bool OnClientCommand(CPlayerSlot, const CCommand&) override
    {
        return true;
    }
    ...
    {
        LogMessage("Connected slot {}", slot);
    }
    ...
    {
        LogMessage("Simulated {} frames this map", frames);
    }

    void Unload() override
    {
        LogMessage("Plugin-owned state can now be released.");
    }

private:
    void StopEvents(const CCommandContext&, const CCommand&)
    {
        if (!StopListeningForGameEvent("round_start"))
            LogWarning("No active round listener was removed.");
    }
