...
public:
    static constexpr PluginInfo Info{
        "Docs Plugins", "KeelS2 documentation", "1.0.0", "Read plugin metadata and dependencies"};
...
private:
    void List(const CCommandContext&, const CCommand&)
    {
        ...
        if (LastResult() != KEEL_RESULT_NOT_FOUND)
            LogError("Plugin list: {}", LastError());

        if (GetPlugin("Docs First Plugin", plugin))
        {
            PluginDetails byId{};

            if (GetPlugin(plugin.handle, byId))
                LogMessage("Dependency resolved: {}", byId.name);
        }
    }
};
}
