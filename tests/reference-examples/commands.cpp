...
{
public:
    static constexpr PluginInfo Info{"Docs Commands", "KeelS2 documentation", "1.0.0", "A managed command and bounded ConVar"};
    bool Load() override
    {
...
private:
    ...
    void Reset(const CCommandContext&, const CCommand& command)
    {
        ...
        if (!existing || !existing.Set(1)) LogWarning("Reset failed: {}", existing.LastError());
    }
