...
int main()
{
    ...
    if (!invocation || invocation.Size() != 2 || invocation[0] != nullptr) return 2;
    raw.size = 0;
    ...
    if (missing || missing.Raw() || missing.Name() || missing.Size() || missing[0]) return 4;

    keels2::Command empty;
    keels2::Command moved(std::move(empty));
    empty = std::move(moved);
    if (empty || moved || empty.Handle() || empty.Reset() != KEEL_RESULT_OK) return 5;
