...
{
public:
    static constexpr PluginInfo Info{"Docs Entities", "KeelS2 documentation", "1.0.0", "Read a caller pawn through a validated entity view"};
    bool Load() override
    {
        ...
        return CreateCommand("keel_docs_health", "Read your current health", &Entities::Health, FCVAR_GAMEDLL | FCVAR_CLIENT_CAN_EXECUTE);
    }
private:
    void Health(const CCommandContext& context, const CCommand&)
    {
        ...
        if (moved.Valid()) LogMessage("The moved view is still valid.");
        const KeelResult result = moved.Reset();
        if (result != KEEL_RESULT_OK) LogWarning("View release returned {}", result);
    }
