...
{
public:
    static constexpr PluginInfo Info{"Docs Profile Hook", "KeelS2 documentation", "1.0.0", "Observe a profile-backed damage target"};
    bool Load() override
    {
...
private:
    Action Before(DamageInfo*, void*) { ++observed; return PLUGIN_CONTINUE; }
    Action After(HookCall<std::int64_t>& call, DamageInfo*, void*)
    {
        if (observed == 1 && call.OriginalCalled())
        {
            const auto result = call.Result();
            if (result) LogMessage("First profile dispatch returned {}", *result);
        }
