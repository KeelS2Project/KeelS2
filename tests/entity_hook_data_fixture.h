...
struct alignas(8) DamageRecord { std::array<std::byte,0x118> bytes; };
inline DamageRecord MakeDamage() {
    DamageRecord value; value.bytes.fill(std::byte{0x5a});
    ...
    for (unsigned i = 0; i < 8; ++i) std::memcpy(value.bytes.data()+offsets[i],fields[i],i >= 6 ? 12 : 4);
    return value;
}
}
