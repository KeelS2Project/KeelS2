...
int main(int argc, char** argv)
{
    try
    {
        if (argc != 4) throw std::runtime_error("consumer dependency fixture-directory");
        ...
        if (!first.OpenWithDependencies(plugin, libraries, error)) throw std::runtime_error(error);
        ...
        if (!second.OpenWithDependencies(second_path, libraries, error)) throw std::runtime_error(error);
        auto raw = second.Symbol("KeelDependencyConsumer");
        int (*invoke)(){};
        static_assert(sizeof(invoke) == sizeof(raw));
        std::memcpy(&invoke, &raw, sizeof(invoke));
        first.Close();
        if (!invoke || invoke() != 42) throw std::runtime_error("shared dependency lost before last consumer");
        second.Close();
