...
namespace {
void Check(bool value, int line) { if (!value) throw std::runtime_error("completion check "+std::to_string(line)); }
...
struct Probe {
    ...
    bool failed{};
    struct Cleanup { Probe* probe; unsigned index; };
    std::array<Cleanup,9> cleanups{};
    std::vector<unsigned> order;
    void Require(bool value) { failed |= !value; }
    static void Complete(void* data) {
        auto& entry = *static_cast<Cleanup*>(data); auto& probe = *entry.probe;
        ...
        ++probe.completed; probe.order.push_back(entry.index);
        if (probe.mode == 5 && entry.index == 4) throw std::runtime_error("cleanup failure");
    }
    static KeelHookAction Callback(KeelHookFrame* frame, void* data) {
        ...
        if (probe.mode == 1) { frame->result.scalar.int32 = 99; return KH_ACTION_SUPERSEDE; }
        if (probe.mode == 2)
            probe.Require(probe.api.set_callback_enabled(0,probe.callback,KEEL_FALSE) == KEEL_RESULT_OK);
        if (probe.mode == 3) throw std::runtime_error("pre failure");
...
struct SchemaEntityServiceTest {
    static void Run() {
        ...
        CHECK(platform::FindLoadedModuleForAddress(reinterpret_cast<void*>(&Probe::Callback),module,error) == platform::ModuleLookup::found);
        service.Authorize(0,module.path,true); service.Authorize(1,module.path,true);
        Probe probe; probe.service = &service; probe.api = service.Api();
        for (unsigned i = 0; i < probe.cleanups.size(); ++i) probe.cleanups[i] = {&probe,i};
        ...
        for (unsigned mode = 0; mode <= 5; ++mode) {
            probe.mode = mode; probe.pre = probe.post = probe.completed = 0; probe.order.clear();
            ...
            if (mode == 4) CHECK(probe.order == std::vector<unsigned>({1,0}));
            if (mode == 5) CHECK(probe.order == std::vector<unsigned>({7,6,5,4,3,2,1,0}));
            CHECK(service.Snapshots()[0].active == 0);
        }
        CHECK(KeelHookService::DeferInternal(reinterpret_cast<KeelHookFrame*>(1),&Probe::Complete,nullptr) == KEEL_RESULT_NOT_READY);
