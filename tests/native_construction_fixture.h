...
void* NativeFactory(const char* name, int index)
{
    ...
    if (g_native_mode == 2) g_base_class.m_pszName = "Unsupported";
    if (g_native_create_callback) g_native_create_callback();
...
void NativeSpawn(void*, const void* values)
{
    ...
    if (g_native_spawn_callback) g_native_spawn_callback();
    if (g_native_mode == 7) {
        g_native_retained = const_cast<void*>(values);
        KeelFixtureKeyValuesRetain(g_native_retained);
        return;
    }
    if (g_native_mode == 3) return;
    if (g_native_mode == 4) throw std::runtime_error("blocked spawn");
    Identity()->m_flags = g_native_mode == 5 ? static_cast<EntityFlags_t>(EF_IS_PRE_SPAWN | EF_SPAWN_IN_PROGRESS) : static_cast<EntityFlags_t>(0);
}
...
public:
    ...
    NativeEnvironment() { table[teleport_slot] = FunctionAddress(&EntityToolTeleport); }
    KeelResult Ready() override { return ready; }
    KeelResult ResolveBase(void*& base) override {
        if (on_base) on_base();
        base = &g_base_class; return ready;
    }
    KeelResult Current(std::uint64_t expected, void*& output, std::uint64_t& captured) noexcept override {
        output = nullptr; captured = 0;
        if (ready != KEEL_RESULT_OK) return ready;
        if (expected && expected != epoch) return KEEL_RESULT_NOT_FOUND;
        output = system; captured = epoch; return KEEL_RESULT_OK;
    }
    ...
    KeelResult TeleportTarget(const char* name, KeelCs2EntityToolBindings& bindings, KeelCs2EntityToolClass& target) override {
        ...
        target = {table.data(),table[teleport_slot]}; return KEEL_RESULT_OK;
    }
};
int RunNativeConstructionChecks()
{
    ...
    struct MemoryScope { ~MemoryScope() { KeelFixtureKeyValuesMemoryStop(); } } memory;
    NativeEnvironment environment;
    keels2::cs2::NativeConstructionBackend backend(environment);
    const auto reset = [&] {
        Reset(); g_native_mode = g_native_spawns = g_native_removes = 0;
        g_native_create_callback = g_native_spawn_callback = {};
        environment.ready = KEEL_RESULT_OK; environment.system = EntitySystem();
        environment.on_base = environment.on_target = {}; ++environment.epoch;
    };
    IdentityType identity{};
    reset(); g_native_mode = 1;
    if (backend.Create("prop_dynamic",identity) != KEEL_RESULT_ALREADY_EXISTS || identity.epoch || g_native_removes || !Identity()->m_pInstance) return 1201;
    for (unsigned mode : {2u,6u}) {
        reset(); g_native_mode = mode;
    ...
    reset(); g_native_create_callback = [&] { ++environment.epoch; environment.system = reinterpret_cast<void*>(1); };
    if (backend.Create("prop_dynamic",identity) != KEEL_RESULT_NOT_FOUND || identity.epoch || g_native_removes) return 1203;
    reset(); environment.on_base = [&] { ++environment.epoch; };
    if (backend.Create("prop_dynamic",identity) != KEEL_RESULT_OK || identity.epoch != environment.epoch || backend.Validate(identity) != KEEL_RESULT_OK) return 1204;
    environment.on_base = {};
    Identity()->m_flags = static_cast<EntityFlags_t>(0);
    if (backend.Cancel(identity) != KEEL_RESULT_NOT_FOUND || g_native_removes) return 1205;
    for (unsigned mode : {0u,3u,4u,5u}) {
        reset(); g_native_mode = mode;
        Store store(backend); std::uint64_t token{}; KeelBool invoked{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1206;
    ...
    for (unsigned mode : {0u,3u}) {
        reset();
        Store store(backend); std::uint64_t token{}; KeelBool invoked{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1208;
        g_native_spawn_callback = [&] { if (store.Cancel(token) != KEEL_RESULT_OK) throw std::runtime_error("close"); };
        g_native_mode = mode;
    ...
    {
        Store store(backend); std::uint64_t token{}; KeelBool invoked{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1210;
        g_native_spawn_callback = [&] { ++environment.epoch; environment.system = reinterpret_cast<void*>(1); store.Reset(); };
        g_native_mode = 3;
        if (store.Spawn(token,invoked) != KEEL_RESULT_NOT_FOUND || !invoked || store.Count() || g_native_removes) return 1211;
    }
    reset();
    {
        Store store(backend); std::uint64_t token{};
        g_native_create_callback = [&] { store.Reset(); };
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_NOT_FOUND || token || store.Count() || g_native_removes != 1) return 1212;
    }
    reset();
    {
        Store store(backend); std::uint64_t token{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1213;
        struct VisitContext { Store* store; std::uint64_t token; } context{&store,token};
        const auto callback = [](void* data, void* const* pointers, std::uint32_t count) -> KeelResult {
            ...
            if (count != 1 || pointers[0] != EntityInstance() || context.store->Cancel(context.token) != KEEL_RESULT_OK || !Identity()->m_pInstance) return KEEL_RESULT_ENGINE_FAILURE;
            return KEEL_RESULT_ALREADY_EXISTS;
        };
        if (store.Visit(token,"CCSPlayerPawn",callback,&context) != KEEL_RESULT_ALREADY_EXISTS || store.Count() || g_native_removes != 1) return 1214;
    }
    reset();
    {
        Store store(backend); std::uint64_t token{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1215;
        ...
        if (store.Teleport(token,request) != KEEL_RESULT_OK || g_tool_calls != 1 || !g_tool_arguments) return 1216;
        environment.on_target = [&] { ++environment.epoch; environment.system = reinterpret_cast<void*>(1); };
    ...
    reset();
    if (backend.Create("prop_dynamic",identity) != KEEL_RESULT_OK) return 1218;
    for (auto flag : {EF_SPAWN_IN_PROGRESS,EF_IS_ANONYMOUS_ALLOCATION}) {
        ...
        if (backend.Validate(identity) != KEEL_RESULT_NOT_FOUND || backend.Cancel(identity) != KEEL_RESULT_NOT_FOUND || g_native_removes) return 1219;
        const KeelCs2EntityIdentity native{identity.index,identity.source2_handle};
        const auto bindings = environment.Bindings(); KeelBool invoked = KEEL_TRUE;
        if (KeelCs2_RemoveCreatedEntity(EntitySystem(),&native,&bindings,&invoked) !=
            (flag == EF_SPAWN_IN_PROGRESS ? KEEL_RESULT_BUSY : KEEL_RESULT_INCOMPATIBLE) || invoked || g_native_removes) return 1228;
    }
    Identity()->m_flags = EF_IS_PRE_SPAWN; environment.ready = KEEL_RESULT_WRONG_THREAD;
    if (backend.Cancel(identity) != KEEL_RESULT_WRONG_THREAD || backend.Validate(identity) != KEEL_RESULT_WRONG_THREAD || g_native_removes) return 1220;
    reset();
    std::vector<std::uint32_t> handles(KEELS2_CS2_ENTITY_CAPACITY);
    if (KeelCs2_SnapshotEntityHandles(nullptr,handles.data(),static_cast<unsigned>(handles.size())) != KEEL_RESULT_INVALID_ARGUMENT ||
        KeelCs2_SnapshotEntityHandles(EntitySystem(),handles.data(),1) != KEEL_RESULT_INVALID_ARGUMENT ||
        KeelCs2_SnapshotEntityHandles(EntitySystem(),handles.data(),static_cast<unsigned>(handles.size())) != KEEL_RESULT_OK ||
        handles[kEntityIndex] != static_cast<unsigned>(Identity()->GetRefEHandle().ToInt()) || handles.back() != UINT32_MAX) return 1221;
    if (KeelFixtureKeyValuesMemoryCount()) return 1222;
    reset();
    {
        Store store(backend); std::uint64_t token{}; KeelBool invoked{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1223;
        g_native_mode = 7;
        ...
        KeelFixtureKeyValuesRelease(g_native_retained); g_native_retained = nullptr;
    }
    for (bool replace : {false,true}) {
        reset(); Store store(backend); std::uint64_t token{}; KeelBool invoked{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1225;
        ...
        g_native_spawn_callback = [&] {
            ...
            if (replace) SetHandle(*Identity(),14);
        };
    ...
    reset();
    if (KeelFixtureKeyValuesMemoryCount()) return 1227;
    {
        Store store(backend); std::uint64_t token{}; KeelBool invoked{};
        if (store.Create("prop_dynamic",token,identity) != KEEL_RESULT_OK) return 1229;
        g_native_spawn_callback = [&] {
            Identity()->m_flags = static_cast<EntityFlags_t>(0);
