...
int main()
{
    ...
    image[rva+8] ^= std::byte{1}; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 6;
    image[rva+8] ^= std::byte{1};
    module.ranges[0].executable = false; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 7;
    module.ranges[0].executable = true; module.ranges[0].readable = false; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 8;
    module.ranges[0].readable = true; --module.ranges[0].size; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 9;
    ++module.ranges[0].size;
    for (auto status : {platform::ModuleLookup::not_found,platform::ModuleLookup::ambiguous}) {
        g_lookup = status; if (!resolve(KEEL_RESULT_INCOMPATIBLE)) return 10;
    }
