...
bool ValidateMessages(const std::string& scenario, const std::string& messages)
{
    ...
    {
        return selected_profile &&
            Contains(messages, "Refreshed plugins: 1 loaded, 0 reloaded, 1 unchanged") &&
            Contains(messages, "Refreshed plugins: 0 loaded, 0 reloaded, 2 unchanged") &&
            Contains(messages, "Refreshed plugins: 0 loaded, 1 reloaded, 0 unchanged, 1 disabled") &&
            Contains(messages, "is still loaded; its file is missing.") &&
            Contains(messages, "plugin reload failed; previous image restored: KeelS2 Basic") &&
    ...
    if (scenario == "plugin_lifecycle")
    {
        ...
        return selected_profile &&
            ...
            Contains(messages, " - disabled") && Contains(messages, "Listing 1 plugins:") &&
    ...
    if (scenario == "source2_service")
    {
        return selected_profile &&
            ...
            Count(messages, "player service contract passed") == 13 &&
    ...
    {
        return selected_profile &&
            Contains(messages, "Version 1.0.0. Loaded") &&
            Contains(messages, "host stopped") &&
    ...
    {
        return selected_profile &&
            Contains(messages, "plugin unload preparation is incomplete; plugin retained: Unload Preparation") &&
            Contains(messages, "exception escaped unload preparation; plugin retained") &&
            Contains(messages, "plugin reloaded transactionally: Unload Preparation") &&
            Count(messages, "cleanup fixture unloaded") == 9 &&
            Count(messages, "cleanup released its ConVar; new registration remained blocked") == 9 &&
            Count(messages, "previous image restored: Unload Preparation") == 2 &&
            Count(messages, "replacement was not activated: Unload Preparation") == 3 &&
            !Contains(messages, "cleanup could not release its ConVar") &&
...
int main(int argument_count, char** arguments)
{
    ...
    const bool plugin_refresh = scenario == "plugin_refresh";
    ...
    if (!missing_server && !unknown_build && !missing_host && !missing_plugin &&
        ...
        !core_commands && !plugin_lifecycle && !plugin_refresh && !command_removal &&
        ...
        !keelhook_shutdown_retry && !unload_preparation && !success)
    {
        return 2;
    }
    ...
    if (client_console)
    {
        ...
        for (const auto command_arguments : {
            ...
            {"keel", "plugins", "cvars"}, {"keel", "plugins", "refresh"},
            {"keel", "plugins", "unload_all"},
    ...
    if (plugin_refresh)
    {
        ...
        const auto new_path = plugin_directory / (std::string("02_lifecycle_first") + plugin_extension);
        const auto dispatch = [&](std::initializer_list<const char*> arguments) {
        ...
        if (!dispatch({"keel", "plugins", "refresh"}) || g_cvar.register_count != registrations ||
            ...
            !g_cvar.HasActive("lifecycle_first") ||
            !dispatch({"keel", "plugins", "refresh"}) || g_cvar.register_count != registrations + 1 ||
            !dispatch({"keel", "plugins", "unload", "Lifecycle First"}) ||
            !dispatch({"keel", "plugins", "refresh"}) || g_cvar.HasActive("lifecycle_first") ||
            g_cvar.register_count != registrations + 1 ||
        ...
        if (error || !dispatch({"keel", "plugins", "refresh"}) ||
            ...
            !CopyFile(failing_plugin_source, basic_path) ||
            !dispatch({"keel", "plugins", "refresh"}) ||
            !dispatch({"keel_test", "after_refresh_rollback"}) ||
            !dispatch({"keel", "plugins", "unload_all"}) || g_cvar.ActiveCount() != 1 ||
    ...
    if (source2_service)
    {
        ...
        input_fixture(1);
        input_fixture(5);
        g_cvar.Dispatch({"s2_check", "players", "input-replaced"});
        input_fixture(4);
        disconnecting(3);
    ...
    if (convar_service)
    {
        ...
        if (!Contains(listing.c_str(), "Listing 4 plugin ConVars:") ||
            ...
            !ContainsInOrder(listing.c_str(), "keels2_test_bool", "keels2_test_float") ||
            !ContainsInOrder(listing.c_str(), "keels2_test_float", "keels2_test_int") ||
            !ContainsInOrder(listing.c_str(), "keels2_test_int", "keels2_test_string"))
        {
    ...
    keels2::platform::DynamicLibrary cleanup_image;
    using CleanupMode = void (*)(unsigned);
