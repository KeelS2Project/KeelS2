...
int main()
{
    ...
    for (unsigned type = 0; type <= 8; ++type) {
        ...
        value.int_value = type == 2 ? 1 : -2147483647;
        value.float_value = -1.25f;
        value.vector_value[0] = 12.5f;
        value.vector_value[1] = -90;
        value.vector_value[2] = 0.125f;
        value.color_value[0] = 3;
        value.color_value[1] = 200;
        value.color_value[2] = 255;
        value.color_value[3] = 0;
        value.entity_handle = 0x12340007;
        auto expected = value;
        expected.string_value = "typed input \xC3\xA9";
        ...
        value.int_value = 99;
        value.vector_value[0] = 99;
        value.color_value[0] = 99;
        ...
        KeelFixtureVariantRelease(native);
        CHECK(!KeelFixtureKeyValuesMemoryCount());
    }
    ...
    for (unsigned type : {9u, UINT32_MAX})
    {
        value.type = type;
        reject();
    }
    ...
    value = Value(1);
    reject();
    std::string long_text(4096, 's');
    value.string_value = long_text.c_str();
    reject();
    long_text.pop_back();
    value.string_value = long_text.c_str();
                      ...
                      std::numeric_limits<float>::quiet_NaN()})
    {
        ...
        for (unsigned type : {5u, 6u})
            ...
            {
                value = Value(type);
                value.vector_value[axis] = bad;
                reject();
            }
    }

    value = Value(8);
    value.entity_handle = UINT32_MAX;
    reject();
    value.entity_handle = 0;
    CHECK(KeelCs2Variant_Build(&value,&native) == KEEL_RESULT_OK && !KeelFixtureVariantInspect(native,&value));
    KeelCs2Variant_Release(native);

    for (unsigned type : {0u,3u,4u,8u}) {
        value = Value(type);
        value.string_value = reinterpret_cast<const char*>(1);
    ...
    for (bool throws : {false,true}) for (unsigned type : {1u,5u,6u,7u}) for (int after : {0,1}) {
                value = Value(type);
                value.string_value = "allocation failure";
                KeelFixtureKeyValuesMemoryFailAfter(after, throws);
                reject(KEEL_RESULT_ENGINE_FAILURE);
