...
int RunOutputContextChecks()
{
    ...
    struct Descriptor
    {
        const char* name;
        std::uint32_t flags;
        std::uint32_t unverified_offset;
    };
    ...
    const auto clear = [&]
    {
        const KeelCs2EntityOutputContext empty{};
        return !std::memcmp(&context, &empty, sizeof(context));
    };
    const auto valid = [&]
    {
        return KeelCs2_ValidateOutputContext(EntitySystem(), &context);
    };

    if (capture(address, g_controller_storage.data(), EntityInstance()) != KEEL_RESULT_OK ||
        valid() != KEEL_RESULT_OK || context.entities[0].index != kEntityIndex || context.entities[1].index != 4 ||
        context.entities[2].index != kEntityIndex ||
        ...
        std::strcmp(context.output_name, "OnTrigger"))
        return 1400;
    ...
    if (capture(address, nullptr, nullptr) != KEEL_RESULT_OK || context.instances[1] || context.instances[2] ||
        valid() != KEEL_RESULT_OK)
        return 1402;

    for (const void* invalid : {static_cast<const void*>(nullptr),reinterpret_cast<const void*>(1)})
        if (capture(invalid, nullptr, nullptr) != KEEL_RESULT_INVALID_ARGUMENT || !clear())
            return 1403;
    ...
    for (const void* invalid : {static_cast<const void*>(EntityInstance()),reinterpret_cast<const void*>(16),
        ...
        if (capture(invalid, nullptr, nullptr) != KEEL_RESULT_NOT_FOUND || !clear())
            return 1404;
    ...
    for (auto flags :
         {EF_IS_PRE_SPAWN, EF_SPAWN_IN_PROGRESS, static_cast<EntityFlags_t>(EF_IS_PRE_SPAWN | EF_SPAWN_IN_PROGRESS)})
    {
        Identity()->m_flags = flags;

        if (capture(address, nullptr, nullptr) != KEEL_RESULT_OK || valid() != KEEL_RESULT_OK)
            return 1406;
    }
    ...
    for (auto flags : {EF_IS_INVALID_EHANDLE,EF_MARKED_FOR_DELETE,EF_DELETE_IN_PROGRESS,EF_IS_CONSTRUCTION_IN_PROGRESS}) {
        ...
        if (capture(address, nullptr, nullptr) != KEEL_RESULT_OK)
            return 1407;

        Identity()->m_flags = flags;

        if (valid() != KEEL_RESULT_NOT_FOUND || capture(address, nullptr, nullptr) != KEEL_RESULT_NOT_FOUND || !clear())
            return 1408;
    }

    reset();

    if (capture(address, g_controller_storage.data(), g_controller_storage.data()) != KEEL_RESULT_OK)
        return 1409;

    auto* controller = &reinterpret_cast<CEntityIdentity*>(g_identity_storage.data())[4];
    controller->m_flags = EF_MARKED_FOR_DELETE;

    if (valid() != KEEL_RESULT_NOT_FOUND)
        return 1410;
    ...
    auto other_type = g_derived_class;
    g_entity_class_info.m_pSchemaBinding = &other_type;
    ...
    reset();
    EntityClass()->m_pClassInfo = nullptr;

    if (capture(address, nullptr, nullptr) != KEEL_RESULT_NOT_FOUND || !clear())
        return 1419;

    reset();
    g_derived_class.m_nAlignment = 0;
    ...
    reset();
    {
        ...
        Identity()->m_designerName = CUtlSymbolLarge(classname.c_str());
        ...
        name.assign(1024, 'n');
        classname.assign(1024, 'c');
        schema.assign(1024, 's');
    }
    ...
    descriptor.name = "OnTrigger";
    reset();
    std::string long_name(128, 'n');
    descriptor.name = long_name.c_str();

    if (capture(address, nullptr, nullptr) != KEEL_RESULT_INCOMPATIBLE || !clear())
        return 1423;

    long_name.pop_back();
    descriptor.name = long_name.c_str();

    if (capture(address, nullptr, nullptr) != KEEL_RESULT_OK || std::strlen(context.output_name) != 127)
        return 1424;
    ...
    if (capture(address, nullptr, nullptr) != KEEL_RESULT_OK || context.output_name[0])
        return 1426;

    const Output unnamed{};
    std::memcpy(g_entity_storage.data() + offset, &unnamed, sizeof(unnamed));
    ...
    descriptor.name = "OnTrigger";
    reset();
    std::string long_class(256, 'c');
    Identity()->m_designerName = CUtlSymbolLarge(long_class.c_str());
    ...
    if (capture(address, nullptr, nullptr) != KEEL_RESULT_OK || std::strlen(context.class_name) != 255)
        return 1429;
    ...
    if (capture(static_cast<std::byte*>(heap) + offset, nullptr, nullptr) != KEEL_RESULT_OK)
        return 1430;

    Identity()->m_pInstance = nullptr;
    ::operator delete(heap, std::align_val_t{16});

    if (valid() != KEEL_RESULT_NOT_FOUND || std::strcmp(context.output_name, "OnTrigger"))
        return 1431;
    ...
    std::memcpy(nested, g_controller_storage.data(), kEntitySize);
    std::memcpy(nested + offset, &output, sizeof(output));
    Identity()->m_pInstance = static_cast<CEntityInstance*>(heap);
    controller = &reinterpret_cast<CEntityIdentity*>(g_identity_storage.data())[4];
    controller->m_pInstance = reinterpret_cast<CEntityInstance*>(nested);
    g_derived_class.m_nSize = static_cast<int>(3 * kEntitySize);

    if (capture(nested + offset, nullptr, nullptr) != KEEL_RESULT_INCOMPATIBLE || !clear())
        return 1434;

    Identity()->m_pInstance = controller->m_pInstance = nullptr;
    ::operator delete(heap, std::align_val_t{16});
