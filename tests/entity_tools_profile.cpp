...
int main() {
    ...
    base_table[slot] = image.data() + entries[2].rva;
    derived_table[slot] = image.data() + end;
    ...
    for (unsigned i = 0; i < 3; ++i) {
        image[entries[i].rva + 15] ^= std::byte{1};
    ...
    base_table[slot] = derived_table[slot];
    ...
    if (cs2::ResolveEntityToolClass(module, "CPhysicsProp", bindings, target, error) != KEEL_RESULT_OK ||
        target.vtable != derived_table.data() || target.teleport != derived_table[slot] ||
        target.teleport == base_table[slot])
        return 11;

    if (cs2::ResolveEntityToolClass(module, "missing", bindings, target, error) != KEEL_RESULT_INCOMPATIBLE ||
        target.vtable || target.teleport)
        return 12;

    derived_table[slot] = image.data();

    if (cs2::ResolveEntityToolClass(module, "CPhysicsProp", bindings, target, error) != KEEL_RESULT_INCOMPATIBLE ||
        target.teleport)
        return 13;

    ++bindings.teleport_slot;

    if (cs2::ResolveEntityToolClass(module, "CBaseEntity", bindings, target, error) != KEEL_RESULT_INVALID_ARGUMENT)
        return 14;
