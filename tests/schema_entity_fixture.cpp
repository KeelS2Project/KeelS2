...
alignas(CSchemaType_Builtin) std::array<std::byte, sizeof(CSchemaType_Builtin)> g_int32_type_storage{};
...
CSchemaClassInfo* DeclaredClass(const char* name)
{
    ...
    if (g_hook_schema_callback && name && std::strcmp(name,"CTakeDamageInfo") == 0) {
        const auto callback = g_hook_schema_callback;
        g_hook_schema_callback = nullptr;
        callback();
    }
    ...
    if (name && std::strcmp(name, "CBaseEntity") == 0)
    {
        if (g_write_schema_callback) {
            const auto callback = g_write_schema_callback;
            g_write_schema_callback = nullptr;
            callback();
        }
    ...
    if (name && std::strcmp(name, "CBaseModelEntity") == 0)
        return &g_tool_model;
    ...
    if (!g_buttons_registered && name && std::strcmp(name, "CInButtonState") == 0)
        return nullptr;

    for (auto* type : {&g_input_pawn_class,
                       &g_movement_class,
                       &g_buttons_class,
                       &g_controller_base,
                       &g_controller_class,
                       ...
                       &g_stat_chainer,
                       &g_hook_component,
                       &g_hook_component_base})
        if (name && type->m_pszName && std::strcmp(name, type->m_pszName) == 0)
            return type;
...
alignas(CGameEntitySystem) std::array<std::byte, sizeof(CGameEntitySystem)> g_entity_system_storage{};
alignas(CEntityIdentity) std::array<std::byte, sizeof(CEntityIdentity) * MAX_ENTITIES_IN_LIST> g_identity_storage{};
alignas(CEntityInstance)
std::array<std::byte, kEntitySize> g_entity_storage{};
alignas(CEntityClass) std::array<std::byte, sizeof(CEntityClass)> g_entity_class_storage{};
...
void Reset()
{
    ...
    g_write_schema_callback = nullptr;
    g_hook_schema_callback = nullptr;
    g_hook_component = g_hook_component_base = {};
...
void InputFixture(std::uint64_t held)
{
    ...
    {
        type->m_eTypeCategory = SCHEMA_TYPE_DECLARED_CLASS;
        type->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
        type->m_pClassInfo = info;
    };
    const auto class_info =
        [](CSchemaClassInfo& info, const char* name, int size, std::uint8_t alignment, SchemaClassFieldData_t* field)
    {
        info = {};
        info.m_pszName = name;
        info.m_nSize = size;
        info.m_nAlignment = alignment;
        info.m_nFieldCount = 1;
        info.m_pFields = field;
    };
    ...
    pointer->m_eTypeCategory = SCHEMA_TYPE_POINTER;
    pointer->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    pointer->m_pObjectType = g_input_movement_type.Get();
    auto* integer = g_input_uint64.Get();
    integer->m_eTypeCategory = SCHEMA_TYPE_BUILTIN;
    integer->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    integer->m_eBuiltinType = SCHEMA_BUILTIN_TYPE_UINT64;
    integer->m_nSize = 8;
    auto* array = g_input_array_type.Get();
    array->m_eTypeCategory = SCHEMA_TYPE_FIXED_ARRAY;
    array->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    array->m_nElementCount = 3;
    array->m_nElementSize = 8;
    array->m_nElementAlignment = 8;
    array->m_pElementType = integer;
    ...
    g_input_pawn_class.m_nBaseClassCount = 1;
    g_input_pawn_class.m_pBaseClasses = &g_input_pawn_base;
    ...
    handle->m_eTypeCategory = SCHEMA_TYPE_ATOMIC;
    handle->m_eAtomicCategory = SCHEMA_ATOMIC_T;
    handle->m_sTypeName = "CHandle<CCSPlayerPawn>";
    handle->m_nSize = 4;
    handle->m_nAlignment = 4;
    ...
    g_controller_class.m_nBaseClassCount = 1;
    g_controller_class.m_pBaseClasses = &g_controller_inherit;
    g_controller_storage.fill(std::byte{});
    g_controller_class_storage.fill(std::byte{});
    ...
    g_controller_info = {};
    g_controller_info.m_pszClassname = "controller";
    g_controller_info.m_pszCPPClassname = "CCSPlayerController";
    g_controller_info.m_pSchemaBinding = &g_controller_class;
    cls->m_pClassInfo = &g_controller_info;
    identity->m_EHandle = CEntityHandle(4, 22);
    identity->m_pInstance = instance;
    identity->m_pClass = cls;
...
void HookDataFixture()
{
    ...
    number->m_eTypeCategory = SCHEMA_TYPE_BUILTIN;
    number->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    number->m_eBuiltinType = SCHEMA_BUILTIN_TYPE_FLOAT32;
    number->m_nSize = 4;
    g_hook_damage_enum = {};
    g_hook_damage_enum.m_pszName = "DamageTypes_t";
    g_hook_damage_enum.m_nSize = g_hook_damage_enum.m_nAlignment = 4;
    auto* enumeration = g_hook_enum.Get();
    enumeration->m_eTypeCategory = SCHEMA_TYPE_DECLARED_ENUM;
    enumeration->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    ...
    vector->m_nSize = 12;
    vector->m_nAlignment = 4;
    auto* entity_type = g_hook_entity_type.Get();
    entity_type->m_eTypeCategory = SCHEMA_TYPE_DECLARED_CLASS;
    entity_type->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    entity_type->m_pClassInfo = &g_base_class;
    auto* handle = g_hook_handle.Get();
    handle->m_eTypeCategory = SCHEMA_TYPE_ATOMIC;
    handle->m_eAtomicCategory = SCHEMA_ATOMIC_T;
    handle->m_sTypeName = "CHandle<CBaseEntity>";
    handle->m_nSize = handle->m_nAlignment = 4;
    handle->m_pTemplateType = entity_type;
    const char* names[]{"m_flDamage",
                        "m_bitsDamageType",
                        "m_iDamageCustom",
                        "m_hInflictor",
                        "m_hAttacker",
    ...
    CSchemaType* types[]{number,
                         enumeration,
                         reinterpret_cast<CSchemaType_Builtin*>(g_int32_type_storage.data()),
                         handle,
                         handle,
                         handle,
                         vector,
    ...
    for (unsigned i = 0; i < 8; ++i)
        g_hook_damage_fields[i] = {names[i], types[i], hook_data_fixture::offsets[i], 0, nullptr};

    g_damage_class = {};
    g_damage_class.m_pszName = "CTakeDamageInfo";
    g_damage_class.m_nSize = 0x118;
    g_damage_class.m_nAlignment = 8;
    g_damage_class.m_nFieldCount = 8;
    g_damage_class.m_pFields = g_hook_damage_fields;
    g_hook_damage = hook_data_fixture::MakeDamage();
    auto* chainer = g_hook_chainer.Get();
    chainer->m_eTypeCategory = SCHEMA_TYPE_ATOMIC;
    chainer->m_eAtomicCategory = SCHEMA_ATOMIC_PLAIN;
    chainer->m_sTypeName = "CNetworkVarChainer";
    chainer->m_nSize = 40;
    chainer->m_nAlignment = 8;
    g_hook_chain = {"__m_pChainEntity",chainer,16,0,nullptr};
    g_hook_component_base = {};
    g_hook_component_base.m_pszName = "CPlayerPawnComponent";
    g_hook_component_base.m_nSize = 72;
    g_hook_component_base.m_nAlignment = 8;
    g_hook_component_base.m_nFieldCount = 1;
    g_hook_component_base.m_pFields = &g_hook_chain;
    g_hook_component_inherit = {0,&g_hook_component_base};
    g_hook_component = {};
    g_hook_component.m_pszName = "CPlayer_WeaponServices";
    g_hook_component.m_nSize = 128;
    g_hook_component.m_nAlignment = 8;
    g_hook_component.m_nBaseClassCount = 1;
    g_hook_component.m_pBaseClasses = &g_hook_component_inherit;
    auto* component_type = g_hook_component_type.Get();
    component_type->m_eTypeCategory = SCHEMA_TYPE_DECLARED_CLASS;
    component_type->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    component_type->m_pClassInfo = &g_hook_component;
    auto* pointer = g_hook_pointer.Get();
    pointer->m_eTypeCategory = SCHEMA_TYPE_POINTER;
    pointer->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    pointer->m_pObjectType = component_type;
    ...
    g_input_pawn_class = {};
    g_input_pawn_class.m_pszName = "CBasePlayerPawn";
    g_input_pawn_class.m_nSize = static_cast<int>(kEntitySize);
    g_input_pawn_class.m_nAlignment = 16;
    g_input_pawn_class.m_nFieldCount = 1;
    g_input_pawn_class.m_pFields = &g_hook_weapon_pointer;
    g_input_pawn_base = {0, &g_base_class};
    g_input_pawn_class.m_nBaseClassCount = 1;
    g_input_pawn_class.m_pBaseClasses = &g_input_pawn_base;
    g_derived_bases[0].m_pClass = &g_input_pawn_class;
...
#include "entity_output_hooks_fixture.h"
int RunHookDataChecks()
{
    ...
    if (KeelCs2_ResolveDamageSchema(&g_schema_system, module, &schema) != KEEL_RESULT_OK)
        return 1001;
    ...
    const auto read = [&]
    {
        return KeelCs2_ReadDamage(&schema, g_hook_damage.bytes.data(), &info);
    };

    if (read() != KEEL_RESULT_OK || info.damage != 42.5f || info.damage_type != 0x80000040 ||
        info.damage_custom != -7 || info.inflictor != 0x6007 || info.attacker != UINT32_MAX || info.ability != 0x7008 ||
        info.force[1] != 2 || info.position[2] != 6)
        return 1002;
    ...
    if (KeelCs2_WriteDamage(&schema, g_hook_damage.bytes.data(), &edit) != KEEL_RESULT_OK ||
        g_hook_damage.bytes != expected.bytes || read() != KEEL_RESULT_OK || info.damage_custom != -7 ||
        info.damage != 12.25f)
        return 1003;
    ...
    edit.damage = 1;
    auto invalid = schema;
    invalid.offsets[7] = invalid.offsets[0];
    ...
    const auto old = g_hook_damage_fields[0];
    g_hook_damage_fields[0].m_nSingleInheritanceOffset = 112;

    if (KeelCs2_ResolveDamageSchema(&g_schema_system, module, &invalid) != KEEL_RESULT_INCOMPATIBLE ||
        invalid.class_info)
        return 1008;

    g_hook_damage_fields[0] = old;
    g_hook_damage_enum.m_nSize = 8;

    if (KeelCs2_ResolveDamageSchema(&g_schema_system, module, &invalid) != KEEL_RESULT_INCOMPATIBLE)
        return 1009;

    g_hook_damage_enum.m_nSize = 4;
    g_hook_damage_fields[2].m_pszName = "missing";

    if (KeelCs2_ResolveDamageSchema(&g_schema_system, module, &invalid) != KEEL_RESULT_NOT_FOUND)
        return 1010;
    ...
    if (read() != KEEL_RESULT_INCOMPATIBLE || info.damage || info.attacker != UINT32_MAX || info.force[0])
        return 1018;

    HookDataFixture();
    info.size = 0;

    if (KeelCs2_ReadDamage(&schema, g_hook_damage.bytes.data(), &info) != KEEL_RESULT_INVALID_ARGUMENT || info.damage)
        return 1019;
    ...
    if (KeelCs2_WeaponMatches(EntitySystem(), &pawn, &weapon, g_hook_weapon.data(), &matches) != KEEL_RESULT_OK ||
        !matches)
        return 1012;

    if (KeelCs2_WeaponMatches(EntitySystem(), &pawn, &weapon, reinterpret_cast<void*>(1), &matches) != KEEL_RESULT_OK ||
        matches)
        return 1013;
    ...
    if (KeelCs2_WeaponMatches(EntitySystem(), &pawn, &weapon, g_hook_weapon.data(), &matches) !=
    ...
    StorePointer(g_hook_weapon.data()+16,EntityInstance());
    auto stale = pawn;
    stale.source2_handle += 0x8000;

    if (KeelCs2_WeaponMatches(EntitySystem(), &stale, &weapon, g_hook_weapon.data(), &matches) !=
            KEEL_RESULT_NOT_FOUND ||
        matches)
        return 1015;
    ...
    if (KeelCs2_ResolveWeaponSchema(&g_schema_system, module, &weapon) != KEEL_RESULT_INCOMPATIBLE || weapon.pawn_class)
        return 1016;

    HookDataFixture();
    g_hook_component_inherit.m_pClass = &g_hook_component;

    if (KeelCs2_ResolveWeaponSchema(&g_schema_system, module, &weapon) != KEEL_RESULT_INCOMPATIBLE)
        return 1017;
...
void TestDamageConstruct(
    void*, void* inflictor, void* attacker, void* ability, float damage, std::uint32_t type, std::int32_t custom)
{
...
{
    ++g_round_calls;
    g_round_reason = reason;
    g_round_delay = delay;
    g_round_team = team ? *team : 0;
    g_round_arguments = rules == g_round_rules_storage.data();

    if (g_round_destroy)
    {
        Identity()->m_pInstance = nullptr;
        g_round_rules_storage.fill(std::byte{});
    }
}
...
int RunRoundChecks()
{
    ...
    auto* object = declared.Get();
    object->m_eTypeCategory = SCHEMA_TYPE_DECLARED_CLASS;
    object->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    object->m_pClassInfo = &g_round_rules_class;
    auto* ptr = pointer.Get();
    ptr->m_eTypeCategory = SCHEMA_TYPE_POINTER;
    ptr->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    ptr->m_pObjectType = object;
    ...
    g_round_proxy_class.m_nSize = static_cast<int>(kEntitySize);
    g_round_proxy_class.m_nAlignment = 16;
    g_round_proxy_class.m_pFields = &g_round_field;
    g_round_proxy_class.m_nFieldCount = 1;
    g_round_rules_class.m_pszName = "CCSGameRules";
    g_round_rules_class.m_nSize = static_cast<int>(g_round_rules_storage.size());
    g_round_rules_class.m_nAlignment = 16;
    ...
    const auto resolve = [&]
    {
        return KeelCs2_ResolveRoundSchema(&g_schema_system, module, &context);
    };
    const auto find = [&]
    {
        return KeelCs2_FindRoundContext(EntitySystem(), &bindings, &context);
    };
    KeelRoundTermination request{sizeof(request), 8, 2.5f, 3, 0};
    const auto apply = [&]
    {
        return KeelCs2_TerminateRound(EntitySystem(), &context, &request, &bindings);
    };
    g_round_calls = 0;
    g_round_destroy = false;
    ...
    for (unsigned reason : {1u,4u,5u,6u,7u,8u,9u,10u,11u,12u,13u,14u,16u,17u,18u,19u,20u,21u,22u}) {
        ...
        if (apply() != KEEL_RESULT_OK || g_round_reason != reason || g_round_team || g_round_delay)
            return 701;
    }

    const auto valid_calls = g_round_calls;

    for (unsigned reason : {0u, 2u, 3u, 15u, 23u, UINT32_MAX})
    {
        request.reason = reason;
    ...
    SetHandle(*Identity(), 12);
    StorePointer(g_entity_storage.data(), other_table.data());

    if (apply() != KEEL_RESULT_INCOMPATIBLE)
        return 708;

    StorePointer(g_entity_storage.data(),proxy_table.data());
    StorePointer(g_round_rules_storage.data(), other_table.data());

    if (apply() != KEEL_RESULT_INCOMPATIBLE)
        return 709;

    StorePointer(g_round_rules_storage.data(),rules_table.data());
    alignas(16) std::array<std::byte, 128> other_rules{};
    StorePointer(other_rules.data(), rules_table.data());
    StorePointer(g_entity_storage.data() + offset, other_rules.data());
    ...
    object->m_pClassInfo = &g_base_class;

    if (resolve() != KEEL_RESULT_INCOMPATIBLE)
        return 713;

    object->m_pClassInfo = &g_round_rules_class;
    ptr->m_eTypeCategory = SCHEMA_TYPE_BUILTIN;
    ...
    g_round_proxy_class.m_pFields = duplicate_fields;
    g_round_proxy_class.m_nFieldCount = 2;
    ...
    if (resolve() != KEEL_RESULT_OK || find() != KEEL_RESULT_OK)
        return 716;

    auto* second = Identity() + 1;
    second->m_EHandle = CEntityHandle(kEntityIndex + 1, 1);
    second->m_pClass = EntityClass();
    second->m_pInstance = reinterpret_cast<CEntityInstance*>(g_controller_storage.data());
    second->m_pInstance->m_pEntity = second;
    second->m_flags = static_cast<EntityFlags_t>(0);

    if (find() != KEEL_RESULT_INCOMPATIBLE)
        return 717;

    second->m_pInstance = nullptr;
    Identity()->m_flags = EF_IS_PRE_SPAWN;

    if (find() != KEEL_RESULT_NOT_FOUND)
        return 718;

    Identity()->m_flags = static_cast<EntityFlags_t>(0);

    if (find() != KEEL_RESULT_OK || g_round_calls != valid_calls)
        return 719;

    g_round_destroy = true;

    if (apply() != KEEL_RESULT_OK || g_round_calls != valid_calls + 1 || Identity()->m_pInstance)
        return 720;
...
void TestStatNotify(void* controller, const NetworkStateChangedData& changed)
{
    ...
    if (g_stat_notify_address)
        std::memcpy(&g_stat_observed, g_stat_notify_address, sizeof(g_stat_observed));

    if (g_stat_destroy)
        Identity()->m_pInstance = nullptr;

    if (g_stat_throw)
        throw std::runtime_error("fixture notification failed after mutation");
}
...
int RunPlayerStatChecks()
{
    ...
    g_controller_class.m_nBaseClassCount = 1;
    g_controller_class.m_pBaseClasses = &root_base;
    ...
    g_stat_match.m_nBaseClassCount = 1;
    g_stat_match.m_pBaseClasses = &match_base;
    ...
    const auto declared = [](CSchemaType_DeclaredClass* type, CSchemaClassInfo* target)
    {
        type->m_eTypeCategory = SCHEMA_TYPE_DECLARED_CLASS;
        type->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
        type->m_pClassInfo = target;
    };
    declared(money_type.Get(), &g_stat_money);
    declared(tracking_type.Get(), &g_stat_tracking);
    declared(match_type.Get(), &g_stat_match);
    declared(chain_type.Get(), &g_stat_chainer);
    SchemaStorage<CSchemaType_Ptr> money_pointer, tracking_pointer;

    const auto pointer = [](CSchemaType_Ptr* type, CSchemaType* target)
    {
        type->m_eTypeCategory = SCHEMA_TYPE_POINTER;
        type->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
        type->m_pObjectType = target;
    };
    pointer(money_pointer.Get(), money_type.Get());
    pointer(tracking_pointer.Get(), tracking_type.Get());
    ...
    g_controller_class.m_nFieldCount = 2;
    g_controller_class.m_pFields = g_controller_fields;
    SchemaClassFieldData_t chain_field{"__m_pChainEntity", chain_type.Get(), 8, 0, nullptr};
    g_stat_base.m_nFieldCount = 1;
    g_stat_base.m_pFields = &chain_field;
    ...
    SchemaClassFieldData_t values[]{{"m_iKills", integer, 0, 0, nullptr},
                                    {"m_iDeaths", integer, 4, 0, nullptr},
                                    {"m_iAssists", integer, 8, 0, nullptr}};

    g_stat_money.m_nFieldCount = g_stat_tracking.m_nFieldCount = 1;
    g_stat_money.m_pFields = &money_field;
    g_stat_tracking.m_pFields = &tracking_field;
    g_stat_values.m_nFieldCount = 3;
    g_stat_values.m_pFields = values;
    ...
    StorePointer(tracking.data(), tracking_table.data());
    StorePointer(money.data() + 8, EntityInstance());
    StorePointer(tracking.data() + 8, EntityInstance());
    const KeelCs2PlayerStatisticsBindings bindings{
        controller_table.data(), money_table.data(), tracking_table.data(), controller_table[kNotifySlot]};
    ...
    const auto resolve = [&](unsigned key)
    {
        return KeelCs2_ResolvePlayerStatSchema(&g_schema_system, module, key, &schema);
    };
    std::int32_t value{};
    const auto read = [&]
    {
        value = 99;
        return KeelCs2_ReadPlayerStat(EntitySystem(), &entity, &schema, &bindings, &value);
    };

    const auto write = [&](std::int32_t next)
    {
        return KeelCs2_WritePlayerStat(EntitySystem(), &entity, &schema, &bindings, next);
    };
    g_stat_notifications = 0;
    g_stat_destroy = g_stat_throw = false;

    for (unsigned key : {1u,2u,4u,8u})
    {
        if (resolve(key) != KEEL_RESULT_OK || read() != KEEL_RESULT_OK || value)
            return 802;
        ...
        if (write(expected) != KEEL_RESULT_OK || g_stat_notifications != notifications)
            return 804;
    }

    if (g_stat_notifications != 4 || resolve(1) != KEEL_RESULT_OK)
        return 805;

    g_stat_notify_address = money.data()+80;

    if (write(INT32_MAX) != KEEL_RESULT_OK || read() != KEEL_RESULT_OK || value != INT32_MAX)
        return 806;

    const auto valid_notifications = g_stat_notifications;

    if (write(-1) != KEEL_RESULT_INVALID_ARGUMENT)
        return 807;

    SetHandle(*Identity(), 13);

    if (read() != KEEL_RESULT_NOT_FOUND || value || write(1) != KEEL_RESULT_NOT_FOUND)
        return 808;

    SetHandle(*Identity(),12);
    StorePointer(money.data() + 8, tracking.data());

    if (read() != KEEL_RESULT_INCOMPATIBLE || value || write(1) != KEEL_RESULT_INCOMPATIBLE)
        return 809;

    StorePointer(money.data()+8,EntityInstance());
    StorePointer(g_entity_storage.data() + money_pointer_offset, nullptr);

    if (read() != KEEL_RESULT_NOT_READY || value)
        return 810;

    StorePointer(g_entity_storage.data()+money_pointer_offset,money.data());
    StorePointer(money.data(), wrong_table.data());

    if (read() != KEEL_RESULT_INCOMPATIBLE || value)
        return 811;

    StorePointer(money.data(),money_table.data());
    const auto original = schema;
    schema.value_offset = 8;

    if (write(1) != KEEL_RESULT_INCOMPATIBLE)
        return 812;

    schema = original;
    schema.value_offset = INT32_MAX;
    ...
    for (int bad : {-1, 81, 128, 8})
    {
        money_field.m_nSingleInheritanceOffset = bad;

        if (resolve(1) != KEEL_RESULT_INCOMPATIBLE)
            return 814;
    }

    money_field.m_nSingleInheritanceOffset = 80;
    integer->m_eBuiltinType = SCHEMA_BUILTIN_TYPE_UINT32;

    if (resolve(1) != KEEL_RESULT_INCOMPATIBLE)
        return 815;

    integer->m_eBuiltinType = SCHEMA_BUILTIN_TYPE_INT32;
    money_pointer.Get()->m_pObjectType = tracking_type.Get();

    if (resolve(1) != KEEL_RESULT_INCOMPATIBLE)
        return 816;

    money_pointer.Get()->m_pObjectType = money_type.Get();
    chain_field.m_nSingleInheritanceOffset = 0;

    if (resolve(1) != KEEL_RESULT_INCOMPATIBLE)
        return 817;

    chain_field.m_nSingleInheritanceOffset = 8;
    g_stat_chainer.m_nSize = 32;

    if (resolve(1) != KEEL_RESULT_INCOMPATIBLE)
        return 818;

    g_stat_chainer.m_nSize = 40;
    SchemaClassFieldData_t duplicates[]{money_field, money_field};
    g_stat_money.m_pFields = duplicates;
    g_stat_money.m_nFieldCount = 2;

    if (resolve(1) != KEEL_RESULT_INCOMPATIBLE)
        return 819;

    g_stat_money.m_pFields = &money_field;
    g_stat_money.m_nFieldCount = 1;
    tracking_field.m_nSingleInheritanceOffset = 250;

    if (resolve(2) != KEEL_RESULT_INCOMPATIBLE)
        return 820;

    tracking_field.m_nSingleInheritanceOffset = 80;
    SchemaStorage<CSchemaType_Atomic> atomic_storage;
    auto* atomic = atomic_storage.Get();
    atomic->m_eTypeCategory = SCHEMA_TYPE_ATOMIC;
    atomic->m_eAtomicCategory = SCHEMA_ATOMIC_PLAIN;
    atomic->m_sTypeName = "CNetworkVarChainer";
    atomic->m_nSize = 40;
    atomic->m_nAlignment = 8;

    struct NameHold
    {
        CSchemaType_Atomic* type;
    ...
    if (resolve(1) != KEEL_RESULT_OK || read() != KEEL_RESULT_OK)
        return 821;
    ...
    for (unsigned invalid : {0u, 3u, 16u, UINT32_MAX})
        if (resolve(invalid) != KEEL_RESULT_INVALID_ARGUMENT || schema.controller_class)
            return 823;

    if (g_stat_notifications != valid_notifications || resolve(1) != KEEL_RESULT_OK)
        return 824;

    g_stat_throw = true;

    if (write(10) != KEEL_RESULT_ENGINE_FAILURE || g_stat_observed != 10 || read() != KEEL_RESULT_OK || value != 10)
        return 825;

    g_stat_throw = false;
    g_stat_destroy = true;

    if (write(11) != KEEL_RESULT_OK || Identity()->m_pInstance || g_stat_observed != 11)
        return 826;

    g_stat_destroy = false;
    g_stat_notify_address = nullptr;
    Reset();
    return 0;
}
...
int RunEntityWriteChecks()
{
    ...
    auto* notify = FunctionAddress(&TestEntityWriteNotify);
    table[kNotifySlot] = notify;
    ...
    if (KeelCs2_ResolveSchemaField(&g_schema_system, module, "CBaseEntity", "m_iHealth", KEELS2_SCHEMA_INT32, &field) !=
        KEEL_RESULT_OK)
        return 451;
    ...
    const auto write = [&](const void* input, unsigned size) {
        ...
        return result == KEEL_RESULT_OK
                   ? KeelCs2_WriteEntityField(EntitySystem(), base, &entity, &field, input, size, notify)
                   : result;
    };
    ...
    if (g_write_notifications != 1 || !g_write_notification_valid)
        return 464;
    ...
    {
        std::int32_t result{};
        std::memcpy(&result, g_entity_storage.data() + kHealthOffset, sizeof(result));
        return result;
    };

    if (write(&value, sizeof(value)) != KEEL_RESULT_OK || health() != value || g_write_notifications != 1 ||
        !g_write_notification_valid)
        return 452;

    if (write(&value, sizeof(value)) != KEEL_RESULT_OK || g_write_notifications != 1)
        return 453;

    auto original = entity;
    ++entity.source2_handle;

    if (write(&value, sizeof(value)) != KEEL_RESULT_NOT_FOUND || g_write_notifications != 1)
        return 454;

    entity = original;
    table[kNotifySlot] = nullptr;
    value = 12;

    if (write(&value, sizeof(value)) != KEEL_RESULT_INCOMPATIBLE || health() != 84)
        return 455;
    ...
    if (write(&value, sizeof(value)) != KEEL_RESULT_INCOMPATIBLE || g_write_notifications != 1)
        return 456;
    ...
    field = original_field;
    field.value_type = KEELS2_SCHEMA_ENTITY_HANDLE;

    if (write(&value, sizeof(value)) != KEEL_RESULT_INVALID_ARGUMENT)
        return 458;

    field = {&g_base_class, static_cast<int>(kHealthOffset), 1, 1, KEELS2_SCHEMA_BOOL};
    const std::uint8_t invalid_bool = 2;

    if (write(&invalid_bool, 1) != KEEL_RESULT_INVALID_ARGUMENT || g_write_notifications != 1)
        return 459;
    ...
    if (write(&nonfinite, 4) != KEEL_RESULT_INVALID_ARGUMENT || g_write_notifications != 1)
        return 460;

    const float finite = 2.5f;

    if (write(&finite, 4) != KEEL_RESULT_OK || g_write_notifications != 2)
        return 461;

    field = {&g_base_class, static_cast<int>(kHealthOffset), 12, 4, KEELS2_SCHEMA_VECTOR3};
    const float vector[]{1,2,3};

    if (write(vector, sizeof(vector)) != KEEL_RESULT_OK || g_write_notifications != 3)
        return 462;
    ...
    g_write_schema_callback = []
    {
        Identity()->m_pInstance = nullptr;
    };

    if (write(&value, sizeof(value)) != KEEL_RESULT_NOT_FOUND || g_write_notifications != 3)
        return 466;
...
void ManagementChangeTeam(void* controller, std::int32_t team)
{
    ...
    g_management_team = team;
    g_management_calls += 1;
}
...
{
    g_management_arguments =
        controller == g_controller_storage.data() && pawn == EntityInstance() && one && !two && !three && !four;
...
int RunPlayerManagementChecks()
{
    ...
    byte->m_eTypeCategory = SCHEMA_TYPE_BUILTIN;
    byte->m_eAtomicCategory = SCHEMA_ATOMIC_INVALID;
    byte->m_eBuiltinType = SCHEMA_BUILTIN_TYPE_UINT8;
    byte->m_nSize = 1;
    g_base_fields[0] = {"m_iTeamNum", byte, static_cast<int>(kHealthOffset + 24), 0, nullptr};
    SchemaBaseClassInfoData_t base{0, &g_base_class};
    g_controller_base.m_nBaseClassCount = 1;
    g_controller_base.m_pBaseClasses = &base;
    ...
    table[change_slot] = FunctionAddress(&ManagementChangeTeam);
    table[respawn_slot] = FunctionAddress(&ManagementRespawn);
    ...
    if (KeelCs2_FindEntityByIndex(EntitySystem(), 4, &controller) != KEEL_RESULT_OK)
        return 400;

    KeelPlayerManagementAction action{sizeof(action), KEELS2_PLAYER_MANAGEMENT_CHANGE_TEAM, 3, 0};
    const auto apply = [&]
    {
        return KeelCs2_ManagePlayer(EntitySystem(), &g_schema_system, module, &controller, &pawn, &action, &bindings);
    };
    const auto prepare = [&]
    {
        return KeelCs2_PrepareRespawn(EntitySystem(), &g_schema_system, module, &controller, &bindings, &pawn);
    };
    g_management_calls = 0;

    if (apply() != KEEL_RESULT_OK || g_management_calls != 1 || g_management_team != 3 || !g_management_arguments)
        return 401;

    action.kind = KEELS2_PLAYER_MANAGEMENT_SWITCH_TEAM;
    action.team = 2;

    if (apply() != KEEL_RESULT_OK || g_management_calls != 11 || g_management_team != 2 || !g_management_arguments)
        return 402;
    ...
    for (const auto team : {-1,0,1,4,256})
    {
        ...
        if (apply() != KEEL_RESULT_INVALID_ARGUMENT || g_management_calls != 11)
            return 403;
    }

    action = {sizeof(action), KEELS2_PLAYER_MANAGEMENT_RESPAWN, 0, 0};

    if (prepare() != KEEL_RESULT_OK || g_management_calls != 1011 || !g_management_arguments ||
        pawn.index != kEntityIndex)
        return 404;

    if (apply() != KEEL_RESULT_OK || g_management_calls != 1111 || !g_management_arguments)
        return 405;

    auto saved = pawn;
    ++pawn.source2_handle;

    if (apply() != KEEL_RESULT_NOT_FOUND || g_management_calls != 1111)
        return 406;
    ...
    if (apply() != KEEL_RESULT_NOT_READY || g_management_calls != 1111)
        return 407;

    g_controller_storage[kHealthOffset + 24] = std::byte{1};

    if (prepare() != KEEL_RESULT_NOT_READY || g_management_calls != 1111)
        return 408;

    g_controller_storage[kHealthOffset + 24] = std::byte{2};
    auto source = controller.source2_handle;
    ++controller.source2_handle;

    if (prepare() != KEEL_RESULT_NOT_FOUND || g_management_calls != 1111)
        return 409;

    controller.source2_handle = source;
    g_controller_class.m_pszName = "NotAPlayerController";

    if (prepare() != KEEL_RESULT_INCOMPATIBLE || g_management_calls != 1111)
        return 410;
...
int RunPlayerActionChecks()
{
    ...
    auto apply = [&]
    {
        return KeelCs2_PlayerAction(EntitySystem(), &g_schema_system, module, &entity, &action, &bindings);
    };
    g_actions.clear();

    if (apply() != KEEL_RESULT_OK || !g_action_arguments || g_action_damage != 5 ||
        g_actions != std::vector<int>({1, 3, 4, 5}))
        return 201;
...
int RunPlayerInputChecks()
{
    ...
    const auto read = [&]
    {
        return KeelCs2_ReadPlayerButtons(EntitySystem(), &g_schema_system, module, &pawn, &buttons, &component);
    };
    ...
    {
        buttons = UINT64_MAX;
        component = reinterpret_cast<void*>(1);
        active = 0;
        return KeelCs2_ReadControllerInput(
            EntitySystem(), &g_schema_system, module, &controller, &buttons, &component, &active);
    };
    ...
    if (controlled() != KEEL_RESULT_INCOMPATIBLE || buttons || component || active != UINT32_MAX)
        return 235;
    ...
    if (controlled() != KEEL_RESULT_NOT_READY || buttons || component || active != UINT32_MAX)
        return 232;
    ...
    if (controlled() != KEEL_RESULT_NOT_FOUND || buttons || component || active != UINT32_MAX)
        return 233;
    ...
    if (!reject(KEEL_RESULT_NOT_FOUND))
        return 228;
...
int RunNativeBridgeChecks()
{
    ...
    auto stale_pointer_entity = entity;
    stale_pointer_entity.source2_handle += 0x8000;
    pointer = reinterpret_cast<void*>(1);

    if (KeelCs2_ResolveEntityPointer(system, &stale_pointer_entity, "CCSPlayerPawn", &pointer) !=
            KEEL_RESULT_NOT_FOUND ||
        pointer)
        return 904;

    const auto saved_alignment = g_derived_class.m_nAlignment;
    ...
    if (invalid_alignment != KEEL_RESULT_INCOMPATIBLE || pointer)
        return 905;
    ...
    if (KeelCs2_CaptureEntity(system, reinterpret_cast<std::byte*>(EntityInstance()) + 1, &captured) !=
        KEEL_RESULT_INVALID_ARGUMENT)
        return 908;
        ...
        reinterpret_cast<CEntityIdentity*>(reinterpret_cast<std::byte*>(Identity()) + 1)})
    {
        ...
        if (status != KEEL_RESULT_NOT_FOUND || captured.index || captured.source2_handle)
            return 909;
    }
    ...
    if (deleted != KEEL_RESULT_NOT_FOUND || captured.index || captured.source2_handle)
        return 910;

    alignas(CEntityInstance) std::array<std::byte, sizeof(CEntityInstance)> copy{};
    std::memcpy(copy.data(), EntityInstance(), copy.size());

    if (KeelCs2_CaptureEntity(system, copy.data(), &captured) != KEEL_RESULT_NOT_FOUND)
        return 911;
    ...
    if (capture_alignment != KEEL_RESULT_INCOMPATIBLE || captured.index || captured.source2_handle)
        return 912;
...
extern "C" KEELS2_SCHEMA_FIXTURE_EXPORT void KeelTest_HookDataFixture(void** damage, void** component)
{
    ...
    if (damage)
        *damage = g_hook_damage.bytes.data();

    if (component)
        *component = g_hook_weapon.data();
}
...
int main()
{
    for (const auto check : {RunHandleChecks,
                             RunPlayerInputChecks,
                             RunEntityWriteChecks,
                             RunPlayerStatChecks,
                             RunRoundChecks,
                             RunPlayerManagementChecks,
                             RunPlayerActionChecks,
                             RunNativeBridgeChecks,
                             RunHookDataChecks,
                             RunEntityToolChecks,
                             RunEntityConstructionChecks,
                             RunNativeConstructionChecks,
                             RunEntityInputChecks,
                             RunNativeInputChecks,
                             RunOutputContextChecks,
                             RunOutputHookChecks})
...
{
    if (mode == 0)
        InputFixture(static_cast<std::uint64_t>(IN_FORWARD) | static_cast<std::uint64_t>(IN_USE));

    if (mode == 1)
        StorePointer(g_entity_storage.data() + g_input_pointer.m_nSingleInheritanceOffset, g_other_movement.data());
    ...
    if (mode == 3)
        StorePointer(g_entity_storage.data() + g_input_pointer.m_nSingleInheritanceOffset, nullptr);
