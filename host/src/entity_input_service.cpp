...
const KeelEntityInputApi& SchemaEntityService::EntityInputApi() const noexcept
{
    return entity_input_api_;
}

KeelResult
SchemaEntityService::InputCapabilitiesEntry(KeelPluginHandle plugin, std::uint32_t* direct, std::uint32_t* queued)
{
    if (direct)
        *direct = 0;
    ...
    const KeelEntityInputRequest* request, KeelBool* invoked)
{
    ...
    if (!target || !request || !invoked || request->size != sizeof(*request))
        return KEEL_RESULT_INVALID_ARGUMENT;

    const auto snapshot = *request;

    if ((snapshot.value.type == KEELS2_INPUT_ENTITY) != (snapshot.value_entity != 0))
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    const KeelEntityInputRequest* request, std::uint32_t* direct, std::uint32_t* queued, KeelBool* invoked)
{
    ...
    } hold{owner->active_native_operations, entity_input_depth_};
    ...
    if (!api.capabilities || !api.dispatch)
        return KEEL_RESULT_UNSUPPORTED;
    ...
    if (!request) {
        ...
        if (result == KEEL_RESULT_OK) {
            ...
            if ((direct_types | queued_types) & ~known)
    ...
    GameEntityIdentity* identities[]{&call.target,&call.activator,&call.caller,&call.value_entity};
    {
        ...
        for (std::size_t i = 0; i < std::size(handles); ++i) if (handles[i]) {
            ...
            *identities[i] = found->second.entity;
            ...
            if (!identities[i]->epoch || identities[i]->epoch != call.target.epoch)
                return KEEL_RESULT_NOT_FOUND;
        }
    }

    call.input = request->input;
    call.value = request->value;
    call.queued = request->queued;
    call.delay = request->delay;
