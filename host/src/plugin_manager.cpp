...
void Host::StartDiscoveredPlugins(
    const std::vector<KeelPluginHandle>& discovered,
    std::unique_lock<std::recursive_mutex>& state_lock)
{
    while (true)
    {
    ...
    if (loaded && !activate_dispatch)
    {
        ...
        if (!PreparePluginPause(*record, state_lock))
        {
            ...
            if (!PreparePluginUnload(*record, state_lock))
            {
                ...
                record->diagnostic = "paused replacement cleanup is incomplete; plugin module was retained";
                Write(KEEL_LOG_ERROR, record->diagnostic + ": " + record->name);
                return record;
            }
        }
    }
...
bool Host::HasRunningDependent(const PluginRecord& plugin, std::string& dependent,
    bool include_retained) const
{
    ...
    for (const auto& candidate : plugins_)
    {
        if (candidate.get() == &plugin || (candidate->state != PluginState::loaded &&
            !(include_retained && candidate->library.IsOpen())))
        {
            continue;
        }
    ...
    std::unique_lock<std::recursive_mutex>& state_lock)
{
    ...
    std::erase_if(disabled_plugins_, [&path](const auto& plugin) {
        return EqualInsensitive(plugin->path.filename().string(), path.filename().string());
    });
    ...
    bool disable)
{
    ...
    if (HasRunningDependent(*plugin, dependent, true))
    {
        Write(
            KEEL_LOG_ERROR,
            "plugin unload is blocked by dependent " + dependent + ": " + plugin->name
        );
        return false;
    }
    ...
    if (disable)
    {
        ...
        plugin->transitioning = false;
        const auto entry = std::find_if(plugins_.begin(), plugins_.end(), [handle](const auto& item) {
            return item->handle == handle;
        });
        disabled_plugins_.push_back(std::move(*entry));
        plugins_.erase(entry);
        std::erase(load_order_, handle);
        std::sort(disabled_plugins_.begin(), disabled_plugins_.end(), [](const auto& left, const auto& right) {
    ...
    std::unique_lock<std::recursive_mutex>& state_lock)
{
    ...
    if (!UnloadPluginCommand(unload_selector, state_lock, false))
    {
    ...
    if (candidate_ready)
    {
        ...
        return candidate->state == (was_paused ? PluginState::paused : PluginState::loaded);
    }
...
bool Host::PluginImageChanged(const PluginRecord& plugin, bool& changed)
{
...
void Host::RefreshPluginsCommand(std::unique_lock<std::recursive_mutex>& state_lock)
{
    ...
    {
        if (plugin->state == PluginState::loaded || plugin->state == PluginState::paused)
            running.push_back(plugin->handle);
        else
            ++failed;
    }
    std::size_t reloaded{};
    ...
    for (const auto handle : running)
    {
        ...
        else if (ReloadPluginCommand(PluginDisplayId(plugin), state_lock))
        {
            ++reloaded;
        }
        else
    ...
    Write(KEEL_LOG_INFO, "Refreshed plugins: " + std::to_string(loaded) + " loaded, " +
        ...
        " missing, " + std::to_string(failed) + " failed.");
}

void Host::UnloadAllPluginsCommand(std::unique_lock<std::recursive_mutex>& state_lock)
{
    BulkPluginOperation operation(bulk_plugin_operation_);
    const std::vector<KeelPluginHandle> order(load_order_.rbegin(), load_order_.rend());
    std::vector<KeelPluginHandle> attempted;
    std::size_t unloaded{};
    bool progress{};
    do
    {
    ...
    bool report)
{
    ...
    if (!PreparePluginPause(*plugin, state_lock))
    {
...
{
    plugin.prepare_pause = nullptr;
    plugin.prepare_pause_data = nullptr;
...
std::vector<PluginRecord*> Host::KnownPlugins() const
{
...
PluginRecord* Host::SelectPlugin(std::string_view selector)
{
    ...
    for (auto* plugin : known)
    {
        if ((plugin->selectable && EqualInsensitive(plugin->name, selector)) ||
            EqualInsensitive(plugin->path.filename().string(), selector))
        {
            return plugin;
        }
    }
...
{
    const auto known = KnownPlugins();
    const auto iterator = std::find(known.begin(), known.end(), plugin);
    return iterator == known.end()
        ? 0
        : static_cast<std::size_t>(std::distance(known.begin(), iterator)) + 1;
}
