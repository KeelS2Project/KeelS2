...
const KeelEntityOutputsApi& SchemaEntityService::EntityOutputsApi() const noexcept
{
    return entity_outputs_api_;
}
...
KeelResult SchemaEntityService::SubscribeOutputEntry(KeelPluginHandle plugin,
                                                     const KeelEntityOutputSpec* spec,
                                                     KeelEntityOutputHandle* output)
{
    if (output)
        *output = 0;

    if (!spec || !output || spec->size != sizeof(*spec))
    ...
    try {
        ...
        if (!copy.callback || !copy.phases || (copy.phases & ~KEELS2_OUTPUT_BOTH) || copy.reserved ||
            !OutputName(copy.class_name, 255, classname) || !OutputName(copy.output_name, 127, name))
            return KEEL_RESULT_INVALID_ARGUMENT;

        copy.class_name = classname.c_str();
        copy.output_name = name.c_str();
        auto* service = active_.load(std::memory_order_acquire);
...
KeelResult SchemaEntityService::UnsubscribeOutputEntry(KeelPluginHandle plugin, KeelEntityOutputHandle handle)
{
    ...
    try {
        ...
        if (found == service->outputs_.end() || found->second->owner != plugin)
            return KEEL_RESULT_NOT_FOUND;

        found->second->enabled = false;
        service->outputs_.erase(found);
...
KeelResult SchemaEntityService::ValidateOutputIdentity(const GameEntityIdentity& identity,
                                                       const std::weak_ptr<Construction>& construction)
{
    const auto pending = construction.lock();

    if (pending && pending->closed.load(std::memory_order_acquire))
        return KEEL_RESULT_NOT_FOUND;

    if (pending && pending->token) {
        if (pending->initializing)
        ...
        if (pending->closed.load(std::memory_order_acquire))
            return KEEL_RESULT_NOT_FOUND;
...
void SchemaEntityService::PruneOutputs(std::uint64_t epoch)
{
    ...
    for (const auto& record : records) {
        ...
        if (stale)
        {
            ...
            outputs_.erase(record->handle);
        }
    }
}

KeelResult SchemaEntityService::ConfigureOutputs(KeelPluginHandle plugin,
                                                 const KeelEntityOutputSpec* spec,
                                                 KeelEntityOutputHandle* output)
{
    std::scoped_lock state_lock(host_.state_mutex_);

    if (!PluginReady(plugin) || outputs_stopping_)
        return KEEL_RESULT_NOT_READY;
    ...
    } hold{owner->active_native_operations, output_depth_};
...

    {
        ...
        const auto result =
            api.start(&adapter_, &host_.keelhook_->Api(), &KeelHookService::DeferInternal, &DispatchOutputsEntry, this);

        if (result != KEEL_RESULT_OK) {
            if (!outputs_started_ && api.stop(&adapter_) == KEEL_RESULT_OK)
                outputs_retained_ = false;
    ...
    const auto count = std::count_if(outputs_.begin(),
                                     outputs_.end(),
                                     [plugin](const auto& pair)
                                     {
                                         return pair.second->owner == plugin;
                                     });

    if (!next_output_ || count >= KEELS2_OUTPUT_MAX_SUBSCRIPTIONS ||
        outputs_.size() >= KEELS2_OUTPUT_MAX_HOST_SUBSCRIPTIONS)
        return KEEL_RESULT_BUSY;

    auto record = std::make_shared<OutputRecord>();
    record->handle = next_output_++;
    record->owner = plugin;
    record->phases = spec->phases;
    record->priority = spec->priority;

    if (spec->entity)
    {
        record->entity = captured.entity;
        record->construction = captured.construction;
    }

    record->class_name = spec->class_name;
    record->output_name = spec->output_name;
    record->callback = spec->callback;
    record->user_data = spec->user_data;
    outputs_.emplace(record->handle, record);
...
bool SchemaEntityService::StopOutputs()
{
    ...
    for (auto& [handle, record] : outputs_)
    {
        static_cast<void>(handle);
        record->enabled = false;
    }
    ...
    if (outputs_starting_ || output_depth_ || !output_invocations_.empty())
        return false;

    if (outputs_retained_) {
        const auto api = host_.adapter_module_ ? host_.adapter_module_->EntityOutputs() : GameAdapterEntityOutputsApi{};

        if (!api.stop || api.stop(&adapter_) != KEEL_RESULT_OK)
            return false;
...
std::uint32_t
SchemaEntityService::DispatchOutputsEntry(const KeelEntityOutputEvent* event, std::uint64_t invocation, void* data)
{
    try
    {
        return static_cast<SchemaEntityService*>(data)->DispatchOutputs(event, invocation);
    }
    catch (...)
    {
        return event && event->phase == KEELS2_OUTPUT_PRE ? KEELS2_OUTPUT_BLOCK : KEELS2_OUTPUT_CONTINUE;
    }
}
...
std::uint32_t SchemaEntityService::DispatchOutputs(const KeelEntityOutputEvent* event, std::uint64_t token)
{
    ...
    if (!event)
    {
        output_invocations_.erase(token);
        return KEELS2_OUTPUT_CONTINUE;
    }

    if (!token || event->size != sizeof(*event) || outputs_stopping_ || !adapter_.IsGameThread())
        return KEELS2_OUTPUT_CONTINUE;

    if (output_depth_ >= KEELS2_OUTPUT_MAX_DEPTH)
    ...
    if (event->phase == KEELS2_OUTPUT_PRE) {
        if (output_invocations_.contains(token) || output_invocations_.size() >= 64)
            return KEELS2_OUTPUT_BLOCK;
        ...
        if (found == output_invocations_.end())
            return KEELS2_OUTPUT_CONTINUE;
    ...
    {
        if (!record->enabled || !(record->phases & event->phase) || !PluginReady(record->owner))
            return;
        ...
        if (!owner || owner->state != PluginState::loaded || owner->transitioning || owner->cleanup_pending ||
            owner->active_native_operations == UINT32_MAX)
            return;
        ...
        std::uint32_t action = UINT32_MAX;

        try
        {
            action = record->callback(&observation, record->user_data);
        }
        catch (...)
        {
        }

        if (action == KEELS2_OUTPUT_BLOCK && event->phase == KEELS2_OUTPUT_PRE)
            block = true;
    ...
    if (event->phase == KEELS2_OUTPUT_PRE)
        for (const auto& record : invocation->callbacks)
            invoke(record);
    else
        for (auto iterator = invocation->callbacks.rbegin(); iterator != invocation->callbacks.rend(); ++iterator)
            invoke(*iterator);
