    ...
    : host_(host), adapter_(adapter), registry_(adapter.PlayerCapacity())
{
    ...
    input_api_ = {sizeof(KeelPlayerInputApi), KEELS2_PLAYER_INPUT_API_VERSION, &InputEntry};
}
...
const KeelPlayerInputApi& PlayerService::InputApi() const noexcept
{
    return input_api_;
}
...
KeelResult PlayerService::Get(KeelPluginHandle plugin, std::int32_t slot, bool next,
    const KeelPlayerConnection* connection, KeelPlayerInfo& player,
    std::unique_lock<std::recursive_mutex>& state_lock, KeelPlayerInput* input)
{
