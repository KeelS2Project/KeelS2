...
bool CopyText(const char* input, std::size_t maximum, std::string& output, bool allow_empty = false)
{
    ...
    if (length > maximum || (!length && !allow_empty))
        return false;
...
public:
    ...
    {
        if (!service_.PluginReady(plugin))
        {
            result_ = KEEL_RESULT_NOT_READY;
            return;
        }

        if (!service_.adapter_.IsGameThread())
        {
            result_ = KEEL_RESULT_WRONG_THREAD;
            return;
        }
        ...
        if (!owner || owner->cleanup_pending || owner->transitioning || owner->active_native_operations == UINT32_MAX ||
            service_.construction_depth_ >= KEELS2_ENTITY_CONSTRUCTION_MAX_DEPTH)
        {
            result_ = KEEL_RESULT_BUSY;
            return;
        }

        owner_ = owner;
        ++owner_->active_native_operations;
        ++service_.construction_depth_;

        if (!service_.host_.adapter_module_)
        {
            result_ = KEEL_RESULT_UNSUPPORTED;
            return;
        }
...
const KeelEntityConstructionApi& SchemaEntityService::EntityConstructionApi() const noexcept
{
    return entity_construction_api_;
}
...
KeelResult SchemaEntityService::ConstructionReadyEntry(KeelPluginHandle plugin)
{
    try
    {
        ...
        return service ? service->ConstructionReady(plugin) : KEEL_RESULT_NOT_READY;
    }
    catch (...)
    {
...
KeelResult SchemaEntityService::CreateEntityEntry(KeelPluginHandle plugin, const char* name, KeelEntityHandle* output)
{
    ...
    {
        auto* service = active_.load(std::memory_order_acquire);
        return service ? service->CreateEntity(plugin, name, output) : KEEL_RESULT_NOT_READY;
    }
    catch (...)
    {
        return KEEL_RESULT_ENGINE_FAILURE;
...
KeelResult
SchemaEntityService::DescribeConstructionEntry(KeelPluginHandle plugin, KeelEntityHandle entity, KeelEntityInfo* info)
{
    ...
    {
        auto* service = active_.load(std::memory_order_acquire);
        return service ? service->DescribeConstruction(plugin, entity, info) : KEEL_RESULT_NOT_READY;
    }
    catch (...)
    {
        return KEEL_RESULT_ENGINE_FAILURE;
...
KeelResult SchemaEntityService::SetConstructionEntry(KeelPluginHandle plugin,
                                                     KeelEntityHandle entity,
                                                     const KeelEntityKeyValue* input)
{
    if (!input || input->size != sizeof(*input) || !entity)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    try {
        ...
        if (!CopyText(value.name, KEELS2_ENTITY_KEY_MAX_NAME, name) || value.type < KEELS2_ENTITY_KEY_STRING ||
            value.type > KEELS2_ENTITY_KEY_COLOR)
            return KEEL_RESULT_INVALID_ARGUMENT;

        if (value.type == KEELS2_ENTITY_KEY_STRING &&
            !CopyText(value.string_value, KEELS2_ENTITY_KEY_MAX_STRING, text, true))
            return KEEL_RESULT_INVALID_ARGUMENT;

        if (value.type == KEELS2_ENTITY_KEY_BOOL && value.int_value != 0 && value.int_value != 1)
            return KEEL_RESULT_INVALID_ARGUMENT;
        ...
        if (value.type == KEELS2_ENTITY_KEY_VECTOR || value.type == KEELS2_ENTITY_KEY_ANGLES)
            for (const float v : value.vector_value)
                if (!std::isfinite(v))
                    return KEEL_RESULT_INVALID_ARGUMENT;
...
KeelResult SchemaEntityService::TeleportConstructionEntry(KeelPluginHandle plugin,
                                                          KeelEntityHandle entity,
                                                          const KeelEntityTeleport* input)
{
    if (!input || input->size != sizeof(*input) || !entity)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    if (!request.flags || (request.flags & ~7u))
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    {
        auto* service = active_.load(std::memory_order_acquire);
        return service ? service->ChangeConstruction(plugin, entity, nullptr, &request, nullptr)
...
KeelResult
SchemaEntityService::SpawnConstructionEntry(KeelPluginHandle plugin, KeelEntityHandle entity, KeelBool* invoked)
{
    if (invoked)
        *invoked = KEEL_FALSE;
    ...
    {
        auto* service = active_.load(std::memory_order_acquire);
        return service ? service->ChangeConstruction(plugin, entity, nullptr, nullptr, invoked) : KEEL_RESULT_NOT_READY;
    }
    catch (...)
    {
        return KEEL_RESULT_ENGINE_FAILURE;
...
KeelResult
SchemaEntityService::ObserveConstructionEntry(KeelPluginHandle plugin, std::uint32_t source, KeelEntityHandle* output)
{
    if (output)
        *output = 0;

    if (!output || source == KEELS2_INVALID_SOURCE2_ENTITY_HANDLE)
        return KEEL_RESULT_INVALID_ARGUMENT;

    try
    {
        auto* service = active_.load(std::memory_order_acquire);
        return service ? service->ObserveConstruction(plugin, source, output) : KEEL_RESULT_NOT_READY;
    }
    catch (...)
    {
        return KEEL_RESULT_ENGINE_FAILURE;
...
KeelResult SchemaEntityService::VisitConstructionEntry(
    KeelPluginHandle plugin, KeelEntityHandle entity, const char* name, KeelEntityAccessCallback callback, void* data)
{
    if (!entity || !callback)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    try {
        ...
        if (!CopyText(name, 255, copied) || !ValidSchemaName(copied.c_str()))
            return KEEL_RESULT_INVALID_ARGUMENT;
...
KeelResult SchemaEntityService::CreateEntity(KeelPluginHandle plugin, const char* input, KeelEntityHandle* output)
{
    ...
    if (operation.Result() != KEEL_RESULT_OK)
    ...
    auto state = std::make_shared<Construction>();
    ...
    KeelEntityHandle handle{};
    {
        ...
        const auto count = std::count_if(entities_.begin(),entities_.end(),[plugin](const auto& item) {
            ...
            return record.owner == plugin && record.construction_owner && record.construction &&
                   !record.construction->closed.load(std::memory_order_acquire) &&
                   (record.construction->initializing || record.construction->token);
        });

        if (count >= KEELS2_ENTITY_CONSTRUCTION_MAX_PENDING || !next_entity_)
            return KEEL_RESULT_BUSY;

        handle = next_entity_++;
        entities_.emplace(handle, EntityRecord{plugin, {}, state, true});
    }
...
KeelResult
SchemaEntityService::DescribeConstruction(KeelPluginHandle plugin, KeelEntityHandle entity, KeelEntityInfo* info)
{
    ConstructionOperation operation(*this,plugin);
    ...
    EntityRecord record;
    {
        ...
        if (found == entities_.end() || !EntityAccessible(found->second, plugin) || !found->second.construction)
            return KEEL_RESULT_NOT_FOUND;

        record = found->second;

        if (record.construction->initializing)
            return KEEL_RESULT_BUSY;

        if (!record.construction->token)
            return KEEL_RESULT_NOT_FOUND;
    }
    ...
    if (record.construction->closed.load(std::memory_order_acquire))
        return KEEL_RESULT_NOT_FOUND;

    if (result != KEEL_RESULT_OK)
        return result;

    if (!SameEntity(record.entity, identity))
        return KEEL_RESULT_INCOMPATIBLE;

    *info = {sizeof(*info), identity.index, identity.source2_handle, 0, identity.epoch};
    return KEEL_RESULT_OK;
}
    ...
    const KeelEntityKeyValue* value, const KeelEntityTeleport* request, KeelBool* invoked)
{
    ...
    std::shared_ptr<Construction> state;
    std::uint64_t token{};
    {
        std::scoped_lock lock(registry_mutex_);
        const auto found = entities_.find(entity);

        if (found == entities_.end() || !EntityAccessible(found->second, plugin) || !found->second.construction_owner ||
            !found->second.construction)
            return KEEL_RESULT_NOT_FOUND;

        state = found->second.construction;

        if (state->initializing || state->busy)
            return KEEL_RESULT_BUSY;
    ...
    if (value)
        return operation.api.set(&adapter_, token, value);

    if (request)
        return operation.api.teleport(&adapter_, token, request);
...
KeelResult
SchemaEntityService::ObserveConstruction(KeelPluginHandle plugin, std::uint32_t source, KeelEntityHandle* output)
{
    ConstructionOperation operation(*this,plugin);
    ...
    bool found_original{};
    {
        ...
        for (const auto& [handle,record] : entities_) {
            ...
            if (record.construction_owner && record.construction && !record.construction->initializing &&
                record.construction->token && !record.construction->closed.load(std::memory_order_acquire) &&
                record.entity.source2_handle == source)
            {
                original = record;
                found_original = true;
                break;
    ...
    if (!SameEntity(original.entity, identity))
        return KEEL_RESULT_INCOMPATIBLE;

    std::scoped_lock lock(registry_mutex_);

    if (original.construction->closed.load(std::memory_order_acquire) || original.construction->token != token ||
        !PluginReady(plugin))
        return KEEL_RESULT_NOT_FOUND;
    ...
    if (count >= KEELS2_ENTITY_CONSTRUCTION_MAX_OBSERVERS || !next_entity_)
        return KEEL_RESULT_BUSY;

    const auto handle = next_entity_++;
    entities_.emplace(handle, EntityRecord{plugin, identity, original.construction, false});
    *output = handle;
    return KEEL_RESULT_OK;
}
...
KeelResult SchemaEntityService::CancelConstruction(const std::shared_ptr<Construction>& state)
{
    ...
    if (incremented)
        ++incremented->active_native_operations;
    ...
    const auto api =
        host_.adapter_module_ ? host_.adapter_module_->EntityConstruction() : GameAdapterEntityConstructionApi{};
