    ...
    std::unique_lock<std::recursive_mutex>& state_lock)
{
    ...
    if (EqualInsensitive(command, "plugins"))
    {
        ...
        else if (EqualInsensitive(subcommand, "cmds") || EqualInsensitive(subcommand, "cvars"))
        {
            if (invocation.argument_count == 2)
            {
                if (EqualInsensitive(subcommand, "cvars"))
                    ShowPluginConVars(nullptr);
                else
            ...
            {
                WriteUsage("keel plugins " + std::string(subcommand) + " [plugin]");
            }
        }
        else if (EqualInsensitive(subcommand, "refresh") || EqualInsensitive(subcommand, "unload_all"))
        {
            ...
            {
                if (refresh)
                    RefreshPluginsCommand(state_lock);
                else
                    UnloadAllPluginsCommand(state_lock);
            }
        }
...
void Host::ShowMainMenu()
{
    ...
    WriteLine("  game     - Show game information");
    WriteLine("  status   - Show host and plugin status");
    WriteLine("  inspect  - Inspect hooks, interfaces, services, and resources");
    WriteLine("  credits  - About KeelS2");
    WriteLine("  version  - Show version and build information");
    WriteLine("  help     - Show this menu or command help");
}
...
void Host::ShowPluginsMenu()
{
    ...
    WriteLine("  list            - List known plugins and their status");
    WriteLine("  cmds [plugin]   - List registered plugin commands");
    WriteLine("  cvars [plugin]  - List registered plugin ConVars");
    ...
    WriteLine("  unload_all      - Unload plugins in dependency order");
    WriteLine("  refresh         - Load new plugins and reload changed plugins");
}
...
void Host::ShowStatus()
{
    ...
    WriteLine(
        "Plugins: " + std::to_string(plugins_.size() + disabled_plugins_.size()) + " total, " +
        std::to_string(loaded) + " loaded, " + std::to_string(invalid) +
        " invalid, " + std::to_string(failed) + " error, " +
        std::to_string(paused) + " paused, " + std::to_string(loading) + " loading, " +
        std::to_string(disabled_plugins_.size()) + " disabled"
    );
...
void Host::ShowPluginConVars(const PluginRecord* plugin)
{
    auto matches = convars_ ? convars_->Snapshots() : std::vector<ConVarService::Snapshot>{};
    std::erase_if(matches, [plugin](const auto& convar) {
        return !convar.created || !convar.owner || (plugin && convar.owner != plugin->handle);
    });
    std::sort(matches.begin(), matches.end(), [](const auto& left, const auto& right) {
        return left.name < right.name;
    });
