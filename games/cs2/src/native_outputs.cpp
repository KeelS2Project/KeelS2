    ...
    host::GameHookDefer defer, host::GameEntityOutputCallback callback, void* data)
{
    ...
    if (callback_)
        return hook_ && function_ == function && defer_ == defer && callback_ == callback && user_data_ == data &&
    ...
    if (result != KEEL_RESULT_OK) {
        ...
        if (hooks_.release_target(0, target_) == KEEL_RESULT_OK)
            target_ = 0;
    }
...
{
    if (std::any_of(pending_.begin(),
                    pending_.end(),
                    [](const auto& value)
                    {
                        return value.owner != nullptr;
                    }))
        return KEEL_RESULT_BUSY;
...
void NativeOutputHooks::Complete(void* data) noexcept
{
    ...
    {
        if (owner->callback_)
            owner->callback_(nullptr, pending.token, owner->user_data_);
    }
    catch (...)
    {
    }
...
{
    if (!frame || !data || frame->size != sizeof(*frame))
        return KH_ACTION_CONTINUE;

    try
    {
        return static_cast<NativeOutputHooks*>(data)->Dispatch(*frame);
    }
    catch (...)
    {
        return frame->phase == KH_PHASE_PRE ? KH_ACTION_SUPERSEDE : KH_ACTION_CONTINUE;
    }
}

KeelHookAction NativeOutputHooks::Dispatch(KeelHookFrame& frame)
{
    if (!environment_.OutputOnThread())
        return KH_ACTION_CONTINUE;

    if (!callback_ || frame.target != target_ || frame.argument_count != 7 || !frame.arguments)
        return KH_ACTION_CONTINUE;

    if (frame.phase == KH_PHASE_POST) {
        const auto found = std::find_if(pending_.begin(),
                                        pending_.end(),
                                        [&](const auto& value)
                                        {
                                            return value.owner && value.frame == &frame;
                                        });
    ...
    if (frame.phase != KH_PHASE_PRE || !next_token_)
        return KH_ACTION_CONTINUE;

    for (unsigned i = 0; i < 7; ++i)
        if (frame.arguments[i].type != (i == 4 ? KH_VALUE_FLOAT32 : KH_VALUE_POINTER))
            return KH_ACTION_CONTINUE;
    ...
    if (environment_.OutputCurrent(0, system, epoch) != KEEL_RESULT_OK || !system || !epoch)
        return KH_ACTION_CONTINUE;
    ...
    if (defer_(&frame, &Complete, &pending) != KEEL_RESULT_OK)
    {
        pending = {};
        return KH_ACTION_CONTINUE;
    }
    ...
    try
    {
        block = callback_(&event, pending.token, user_data_) == KEELS2_OUTPUT_BLOCK;
    }
    catch (...)
    {
        block = true;
    ...
    if (environment_.OutputCurrent(epoch, current, current_epoch) != KEEL_RESULT_OK || current != system ||
        current_epoch != epoch || KeelCs2_ValidateOutputContext(current, &pending.native) != KEEL_RESULT_OK)
        block = true;
