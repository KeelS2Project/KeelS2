...
KeelResult NativeConstructionBackend::Ready() { return environment_.Ready(); }
KeelResult NativeConstructionBackend::Current(const host::GameEntityIdentity& entity, void*& system)
{
    system = nullptr;
    if (!entity.epoch) return KEEL_RESULT_NOT_FOUND;
    std::uint64_t epoch{};
...
KeelResult NativeConstructionBackend::Create(const char* name, host::GameEntityIdentity& output)
{
    ...
    if (result != KEEL_RESULT_OK) return result;
    if (!system || !epoch) return KEEL_RESULT_NOT_READY;
    ...
    if (result != KEEL_RESULT_OK) return result;
    if (native.index < 0 || static_cast<std::size_t>(native.index) >= before.size()) return KEEL_RESULT_INCOMPATIBLE;
    if (before[static_cast<std::size_t>(native.index)] == native.source2_handle) return KEEL_RESULT_ALREADY_EXISTS;
...
KeelResult NativeConstructionBackend::Spawn(const host::GameEntityIdentity& entity, const void* values, KeelBool& invoked)
{
    invoked = KEEL_FALSE;
    void* base{};
    auto result = environment_.ResolveBase(base);
...
KeelResult NativeConstructionBackend::Teleport(const host::GameEntityIdentity& entity, const KeelEntityTeleport& request)
{
    void* base{};
    auto result = environment_.ResolveBase(base);
    if (result != KEEL_RESULT_OK) return result;
    ...
    if (result != KEEL_RESULT_OK || current != system) return result == KEEL_RESULT_OK ? KEEL_RESULT_NOT_FOUND : result;
...
{
    if (!callback) return KEEL_RESULT_INVALID_ARGUMENT;
    void* system{};
    auto result = Current(entity,system);
    if (result != KEEL_RESULT_OK) return result;
    const KeelCs2EntityIdentity native{entity.index,entity.source2_handle};
