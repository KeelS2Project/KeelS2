    ...
    const char* input, const void* native_value, KeelBool queued, float delay, KeelBool* invoked)
{
    ...
    for (std::size_t i = 0; i < instances.size(); ++i) if (identities[i]) {
        ...
        if (!found || found->GetEntityIndex().Get() != selected.index)
            return KEEL_RESULT_NOT_FOUND;
...
extern "C" KeelResult KeelCs2_CaptureEntity(void* entity_system, const void* instance, KeelCs2EntityIdentity* output)
{
    ...
    try
    {
        ...
        for (std::size_t chunk_index = 0; chunk_index < std::size(system->m_EntityList.m_pIdentityChunks); ++chunk_index)
        {
            ...
            if (address < begin || address - begin >= bytes || (address - begin) % sizeof(CEntityIdentity))
                continue;
            ...
            if (!identity || identity != backlink || identity->m_pInstance != instance)
                return KEEL_RESULT_NOT_FOUND;
...
extern "C" KeelResult KeelCs2_ResolveEntityWriteClass(void* schema_system, const char* module, void** base_class)
{
    ...
    if (!schema_system || !module || !*module || !base_class)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    const void* value, std::uint32_t value_size, void* notify)
{
    ...
    if (field->value_type == KEELS2_SCHEMA_FLOAT32 || field->value_type == KEELS2_SCHEMA_VECTOR3)
        ...
        {
            float component{};
            std::memcpy(&component, snapshot.data() + at, sizeof(component));
    ...
    {
        double number{};
        std::memcpy(&number, snapshot.data(), sizeof(number));
    ...
    try
    {
        ...
        if (field->value_size - 1 > UINTPTR_MAX - address)
            return KEEL_RESULT_INCOMPATIBLE;
        ...
        if (!table || table[KEELS2_CS2_NETWORK_STATE_CHANGED_SLOT] != notify)
            return KEEL_RESULT_INCOMPATIBLE;
    ...
    const KeelCs2PlayerActionBindings* bindings)
{
    ...
    try
    {
        ...
        if (action->damage > 0)
        {
            ...
            std::vector<std::max_align_t> storage((bindings->damage_info_size + sizeof(std::max_align_t) - 1) /
                                                  sizeof(std::max_align_t));

            auto* world = IdentityByIndex(static_cast<CEntitySystem*>(entity_system), 0);
            void* source = world ? world->m_pInstance : nullptr;
            NativeActionFunction<Construct>(bindings->damage_construct)(
                storage.data(), source, source, nullptr, action->damage, 0, 0);
...
#endif
    if (table != bindings->controller_vtable || table[change_slot] != bindings->change_team ||
        table[respawn_slot] != bindings->respawn)
        return KEEL_RESULT_INCOMPATIBLE;
...
KeelResult RoundPointerOffset(const CSchemaClassInfo* proxy, const CSchemaClassInfo* rules, std::int32_t& offset)
{
    if (!ValidClass(proxy) || !ValidClass(rules) || !proxy->m_pszName || !rules->m_pszName ||
        ...
        proxy->m_nAlignment < alignof(void*) || rules->m_nAlignment < alignof(void*) ||
        rules->m_nSize < static_cast<int>(sizeof(void*)))
        return KEEL_RESULT_INCOMPATIBLE;
    ...
    for (std::uint32_t i = 0; i < proxy->m_nFieldCount; ++i)
    {
        ...
        if (!field.m_pszName || std::strcmp(field.m_pszName, "m_pGameRules"))
            continue;
    ...
    if (!object || object->m_eTypeCategory != SCHEMA_TYPE_DECLARED_CLASS ||
...
{
    if (!identity || identity->m_pClass->GetSchemaBinding() != context.proxy_class)
        return KEEL_RESULT_INCOMPATIBLE;
    ...
    if (table != bindings.proxy_vtable)
        return KEEL_RESULT_INCOMPATIBLE;
...
extern "C" KeelResult KeelCs2_ResolveRoundSchema(void* schema_system, const char* module, KeelCs2RoundContext* context)
{
    ...
    if (!schema_system || !module || !*module || !context)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    try
    {
        ...
        context->proxy_class = proxy;
        context->rules_class = rules;
        context->pointer_offset = offset;
        return KEEL_RESULT_OK;
    }
    catch (...)
    {
...
{
    if (!entity_system || !bindings || !bindings->rules_vtable || !bindings->proxy_vtable || !bindings->terminate ||
        !context)
        return KEEL_RESULT_INVALID_ARGUMENT;

    context->proxy = {};
    context->proxy_instance = nullptr;
    ...
    try
    {
        ...
        if (offset != context->pointer_offset)
            return KEEL_RESULT_INCOMPATIBLE;
        ...
        for (std::int32_t i = 0; i < MAX_TOTAL_ENTITIES; ++i)
        {
            ...
            if (!type || !type->m_pszName || std::strcmp(type->m_pszName, "CCSGameRulesProxy"))
                continue;

            if (type != context->proxy_class || selected)
                return KEEL_RESULT_INCOMPATIBLE;
        ...
        context->proxy = {selected->GetEntityIndex().Get(),
                          static_cast<std::uint32_t>(selected->GetRefEHandle().ToInt())};

        context->proxy_instance = selected->m_pInstance;
        context->rules = rules;
        return KEEL_RESULT_OK;
    }
    catch (...)
...
{
    if (!entity_system || !context || !context->proxy_instance || !context->rules || !input ||
        input->size != sizeof(*input) || !bindings || !bindings->rules_vtable || !bindings->proxy_vtable ||
        !bindings->terminate)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    KeelCs2EntityIdentity* prepared_pawn)
{
    ...
    if (!schema || !module || !*module || !prepared_pawn)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    const KeelCs2EntityIdentity* pawn, uint64_t* buttons, void** component)
{
    ...
    try
    {
        ...
        if (!category(state->m_pType, SCHEMA_TYPE_DECLARED_CLASS))
            return KEEL_RESULT_INCOMPATIBLE;
        ...
        const auto fits = [](const CSchemaClassInfo* type,
                             const SchemaClassFieldData_t* member,
                             std::size_t size,
        ...
        const auto offset = static_cast<std::uint64_t>(state->m_nSingleInheritanceOffset) +
                            static_cast<std::uint64_t>(masks->m_nSingleInheritanceOffset);
        ...
        if (current != movement_object)
            return KEEL_RESULT_NOT_FOUND;

        struct Mapping
        {
            InputBitMask_t source;
            uint64_t target;
        };
        ...
        for (const auto& mapping : mappings)
            if (held & static_cast<uint64_t>(mapping.source))
    ...
    const KeelCs2EntityIdentity* controller, uint64_t* buttons, void** component, uint32_t* pawn_handle)
{
    ...
    {
        KeelCs2SchemaField field{};
        auto result = KeelCs2_ResolveSchemaField(
            schema_system, module, "CBasePlayerController", "m_hPawn", KEELS2_SCHEMA_ENTITY_HANDLE, &field);
        ...
        if (current != active || KeelCs2_ValidateEntity(entity_system, &pawn) != KEEL_RESULT_OK)
            return KEEL_RESULT_NOT_FOUND;

        *buttons = held;
        *component = movement;
        *pawn_handle = pawn.source2_handle;
        return KEEL_RESULT_OK;
    }
    catch (...)
    {
...
const SchemaClassFieldData_t& StatField(const CSchemaClassInfo* type, const char* name)
{
    ...
    for (std::uint32_t i = 0; i < type->m_nFieldCount; ++i)
    {
        ...
        if (!field.m_pszName || std::strcmp(field.m_pszName, name))
            continue;
...
std::uint32_t StatSpan(const CSchemaClassInfo* type, std::int32_t offset, std::uint32_t size, std::uint32_t alignment)
{
    ...
    StatRequire(start % alignment == 0 &&
                static_cast<std::uint64_t>(start) + size <= static_cast<std::uint64_t>(type->m_nSize));
...
{
    StatRequire(type && type->m_eTypeCategory == SCHEMA_TYPE_DECLARED_CLASS &&
...
std::uint32_t StatBase(const CSchemaClassInfo* derived, const CSchemaClassInfo* base)
{
    ...
    bool found{};
    std::uint64_t offset{};
    StatRequire(FindBaseOffset(derived, base, 0, path, 0, visited, found, offset) == HierarchyResult::found && found &&
                offset <= INT32_MAX);
...
{
    return key == KEELS2_PLAYER_STAT_MONEY ? "CCSPlayerController_InGameMoneyServices"
    ...
    const KeelCs2PlayerStatisticsBindings& bindings, void*& controller)
{
    ...
    StatRequire(ValidClass(root) && ValidClass(type) && root->m_pszName && type->m_pszName &&
                std::strcmp(root->m_pszName, "CCSPlayerController") == 0 &&
                std::strcmp(type->m_pszName, StatComponentName(schema.key)) == 0);
    ...
    KeelCs2PlayerStatSchema* output)
{
    ...
    if (!output || !schema_system || !module || !*module || !StatKey(key))
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    try
    {
        ...
        const auto& pointer = StatField(
            controller, key == KEELS2_PLAYER_STAT_MONEY ? "m_pInGameMoneyServices" : "m_pActionTrackingServices");

        StatRequire(pointer.m_pType->m_eTypeCategory == SCHEMA_TYPE_POINTER &&
                    pointer.m_pType->m_eAtomicCategory == SCHEMA_ATOMIC_INVALID);
        ...
        StatRequire(field.m_pType->m_eTypeCategory == SCHEMA_TYPE_BUILTIN &&
                    field.m_pType->m_eAtomicCategory == SCHEMA_ATOMIC_INVALID);
    ...
    catch (const StatFailure& failure)
    {
    ...
    const KeelCs2PlayerStatSchema* schema, const KeelCs2PlayerStatisticsBindings* bindings, std::int32_t* output)
{
    ...
    if (!entity || !schema || !bindings || !output)
        return KEEL_RESULT_INVALID_ARGUMENT;
...
{
    if (!entity || !schema || !bindings || value < 0)
        return KEEL_RESULT_INVALID_ARGUMENT;
...
bool DamageNumbers(float damage, const float* force, const float* position)
{
    ...
    for (unsigned i = 0; i < 3; ++i)
        if (!std::isfinite(force[i]) || !std::isfinite(position[i]))
            return false;
...
extern "C" KeelResult KeelCs2_ResolveDamageSchema(void* system, const char* module, KeelCs2DamageSchema* output)
{
    ...
    {
        auto* scope = reinterpret_cast<ISchemaSystemTypeScope*>(
            static_cast<ISchemaSystem*>(system)->FindTypeScopeForModule(module));

        StatRequire(scope != nullptr,KEEL_RESULT_NOT_FOUND);
        auto* type = StatClass(scope,"CTakeDamageInfo");
        KeelCs2DamageSchema result{};
        result.class_info = type;
...
extern "C" KeelResult KeelCs2_ReadDamage(const KeelCs2DamageSchema* schema, const void* record, KeelDamageInfo* output)
{
    ...
    try
    {
        ...
        for (unsigned i = 0; i < 8; ++i)
            std::memcpy(values[i], address + schema->offsets[i], kDamageSizes[i]);

        if (!DamageNumbers(result.damage, result.force, result.position))
            return KEEL_RESULT_INCOMPATIBLE;
...
{
    if (!schema || !input || input->size != sizeof(*input))
        return KEEL_RESULT_INVALID_ARGUMENT;

    const auto edit = *input;

    if (edit.reserved || edit.damage < 0 || !DamageNumbers(edit.damage, edit.force, edit.position))
        return KEEL_RESULT_INVALID_ARGUMENT;
...
extern "C" KeelResult KeelCs2_ResolveWeaponSchema(void* system, const char* module, KeelCs2WeaponSchema* output)
{
    ...
    try
    {
        ...
        const auto pointer_offset =
            StatBase(pawn, base) + StatSpan(base, pointer.m_nSingleInheritanceOffset, sizeof(void*), alignof(void*));
        ...
        const auto chain_offset =
            StatBase(component, component_base) + StatSpan(component_base, chain.m_nSingleInheritanceOffset, 40, 8);
    ...
    const KeelCs2WeaponSchema* schema, const void* candidate, KeelBool* matches)
{
    ...
    if (!system || !pawn || !schema || !candidate || !matches)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    bool require_pre_spawn) noexcept
{
    ...
    if (handle.GetEntryIndex() != entity.index)
        return nullptr;
...
KeelResult CheckCreatedClass(CEntityIdentity* identity, void* base)
{
    ...
    StatRequire(instance % type->m_nAlignment == 0 &&
                static_cast<std::uint32_t>(type->m_nSize) >= sizeof(CEntityInstance) &&
    ...
    const KeelCs2EntityConstructionBindings* bindings, const void* key_values, KeelBool* invoked, bool spawn)
{
    ...
    if (!system || !entity || (spawn && !base) || !bindings || !bindings->create || !bindings->spawn ||
        !bindings->remove || !invoked)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    try {
        ...
        if (!spawn && (identity->m_flags & EF_SPAWN_IN_PROGRESS))
            return KEEL_RESULT_BUSY;

        if (!spawn && (identity->m_flags & EF_IS_ANONYMOUS_ALLOCATION))
            return KEEL_RESULT_INCOMPATIBLE;
        ...
        *invoked = KEEL_TRUE;
        ...
        if (spawn)
            NativeActionFunction<void (*)(void*, const void*)>(bindings->spawn)(instance, key_values);
    ...
    try {
        ...
        for (std::size_t chunk_index = 0; chunk_index < std::size(registry->m_EntityList.m_pIdentityChunks); ++chunk_index) {
            ...
            for (std::size_t offset = 0; offset < MAX_ENTITIES_IN_LIST; ++offset) {
                ...
                if (candidate->m_pInstance != instance)
                    continue;
                ...
                if (!CreatedIdentity(registry, entity, false))
                    return KEEL_RESULT_NOT_FOUND;
    ...
    void* base, KeelBool require_pre_spawn)
{
    ...
    try
    {
        return CheckCreatedClass(
            CreatedIdentity(static_cast<CEntitySystem*>(system), *entity, require_pre_spawn != KEEL_FALSE), base);
    }
    catch (const StatFailure& failure)
    {
        return failure.result;
...
const CSchemaClassInfo* OutputOwnerType(const CEntityIdentity& identity, const void* output) noexcept
{
    ...
    if (!descriptor || !descriptor->m_pClassInfo)
        return nullptr;

    const auto* type = descriptor->GetSchemaBinding();

    if (!ValidClass(type) || static_cast<std::uint32_t>(type->m_nSize) < sizeof(CEntityInstance) + 24)
        return nullptr;
...
bool CopyOutputName(char (&destination)[N], const char* source, bool allow_empty = false) noexcept
{
    ...
    if (length == N || (!length && !allow_empty))
        return false;
    ...
    try {
        ...
        for (std::size_t chunk_index = 0; chunk_index < std::size(registry->m_EntityList.m_pIdentityChunks); ++chunk_index) {
            ...
            for (std::size_t slot = 0; slot < MAX_ENTITIES_IN_LIST; ++slot) {
                ...
                if (!candidate->m_pInstance)
                    continue;

                const KeelCs2EntityIdentity identity{static_cast<std::int32_t>(chunk_index*MAX_ENTITIES_IN_LIST+slot),
                    static_cast<std::uint32_t>(candidate->GetRefEHandle().ToInt())};

                if (!CreatedIdentity(registry, identity, false))
                    continue;

                if (candidate->m_pInstance == activator)
                {
                    captured.entities[1] = identity;
                    captured.instances[1] = activator;
                }

                if (candidate->m_pInstance == caller)
                {
                    captured.entities[2] = identity;
                    captured.instances[2] = caller;
                }
                ...
                owner = candidate;
                captured.entities[0] = identity;
                captured.instances[0] = owner->m_pInstance;
                captured.owner_class = owner->m_pClass;
                captured.owner_schema = type;
            }
        }

        if (!owner || (activator && !captured.instances[1]) || (caller && !captured.instances[2]))
            return KEEL_RESULT_NOT_FOUND;
        ...
        if (descriptor)
            std::memcpy(&name, descriptor, sizeof(name));

        if (!CopyOutputName(captured.output_name, name, true))
            return KEEL_RESULT_INCOMPATIBLE;
...
{
    if (!system || !context || !context->output || !context->instances[0])
        return KEEL_RESULT_INVALID_ARGUMENT;

    auto* registry = static_cast<CEntitySystem*>(system);

    for (std::size_t i = 0; i < std::size(context->instances); ++i) {
        if (!context->instances[i])
            continue;

        auto* identity = CreatedIdentity(registry,context->entities[i],false);

        if (!identity || identity->m_pInstance != context->instances[i])
            return KEEL_RESULT_NOT_FOUND;
...
extern "C" KeelResult KeelCs2_SnapshotEntityHandles(void* system, uint32_t* handles, uint32_t count)
{
    ...
    if (!system || !handles || count != KEELS2_CS2_ENTITY_CAPACITY)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    const KeelCs2EntityConstructionBindings* bindings)
{
    ...
    if (identity->m_flags & EF_SPAWN_IN_PROGRESS)
        return KEEL_RESULT_BUSY;

    if (!(identity->m_flags & EF_IS_PRE_SPAWN))
        return KEEL_RESULT_OK;

    if (identity->m_flags & EF_IS_ANONYMOUS_ALLOCATION)
        return KEEL_RESULT_INCOMPATIBLE;
    ...
    const char* class_name, void** output)
{
    ...
    if (!system || !entity || !class_name || !*class_name || !output)
        return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    KeelCs2EntityToolContext* context, bool created)
{
    ...
    try {
        ...
        StatRequire(std::strcmp(required->m_pszName, "CBaseEntity") == 0 ||
                    (!created && std::strcmp(required->m_pszName, "CBaseModelEntity") == 0));
        ...
        while (length < sizeof(context->class_name) && type->m_pszName[length])
            ++length;

        StatRequire(length && length < sizeof(context->class_name));
        context->class_info = type;
...
KeelResult ApplyEntityTool(void* system,
                           const KeelCs2EntityIdentity* entity,
                           const KeelCs2EntityToolContext* context,
                           const KeelCs2EntityToolBindings* bindings,
                           const KeelCs2EntityToolClass* target,
                           std::uint32_t kind,
                           ...
                           bool created)
{
    ...
    try {
        ...
        if (current.class_info != context->class_info ||
            std::memcmp(current.class_name, context->class_name, sizeof(current.class_name)))
            return KEEL_RESULT_INCOMPATIBLE;
        ...
        if (table != target->vtable || table[slot] != target->teleport)
            return KEEL_RESULT_INCOMPATIBLE;
...
{
    return ApplyEntityTool(
        system, entity, context, bindings, target, KEELS2_ENTITY_TOOL_TELEPORT, request, nullptr, true);
}
