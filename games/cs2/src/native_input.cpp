...
KeelResult NativeInputBackend::Dispatch(const host::GameEntityInputRequest& request, KeelBool& invoked)
{
    ...
    for (std::size_t i = 0; i < native.size(); ++i) {
        ...
        if (!identity.epoch) {
            if (identity.index || identity.source2_handle)
                return KEEL_RESULT_INVALID_ARGUMENT;

            continue;
        }

        if (identity.index < 0 || static_cast<std::uint32_t>(identity.index) >= KEELS2_CS2_ENTITY_CAPACITY ||
            identity.source2_handle == KEELS2_INVALID_SOURCE2_ENTITY_HANDLE)
            return KEEL_RESULT_INVALID_ARGUMENT;

        if (identity.epoch != snapshot.target.epoch)
    ...
    if (!((snapshot.queued ? queued : direct) & (1u << copied.value.type)))
        return KEEL_RESULT_UNSUPPORTED;

    const auto bindings = environment_.InputBindings();
    KeelCs2VariantValue value{};
    value.size = sizeof(value);
    value.type = copied.value.type;
    value.string_value = copied.value.string_value;
    value.int_value = copied.value.int_value;
    value.float_value = copied.value.float_value;
    ...
    if (!system || epoch != snapshot.target.epoch)
        return KEEL_RESULT_NOT_FOUND;
