...
extern "C" KeelResult KeelCs2Variant_Build(const KeelCs2VariantValue* value, void** output)
{
    ...
    if (!output || !value || value->size != sizeof(*value))
    ...
    switch (copy.type) {
        ...
        case KEELS2_CS2_VARIANT_STRING:
            ...
            while (string_length <= KEELS2_CS2_VARIANT_MAX_STRING && copy.string_value[string_length])
                ++string_length;

            if (string_length > KEELS2_CS2_VARIANT_MAX_STRING)
                return KEEL_RESULT_INVALID_ARGUMENT;

            break;

        case KEELS2_CS2_VARIANT_BOOL:
            if (copy.int_value != 0 && copy.int_value != 1)
                return KEEL_RESULT_INVALID_ARGUMENT;

            break;

        case KEELS2_CS2_VARIANT_FLOAT:
            if (!std::isfinite(copy.float_value))
                return KEEL_RESULT_INVALID_ARGUMENT;
        ...
        case KEELS2_CS2_VARIANT_ANGLES:
            for (float component : copy.vector_value)
        ...
        case KEELS2_CS2_VARIANT_ENTITY:
            if (copy.entity_handle == INVALID_EHANDLE_INDEX)
                return KEEL_RESULT_INVALID_ARGUMENT;
    ...
    try {
        ...
        switch (copy.type) {
            ...
            case KEELS2_CS2_VARIANT_STRING:
                CopyPayload(*result, FIELD_CSTRING, copy.string_value, string_length + 1);
                break;

            case KEELS2_CS2_VARIANT_BOOL:
                *result = copy.int_value != 0;
                break;

            case KEELS2_CS2_VARIANT_INT32:
                *result = copy.int_value;
                break;

            case KEELS2_CS2_VARIANT_FLOAT:
                *result = copy.float_value;
                break;

            case KEELS2_CS2_VARIANT_VECTOR: {
                const Vector vector(copy.vector_value[0],copy.vector_value[1],copy.vector_value[2]);
                CopyPayload(*result, FIELD_VECTOR, &vector, sizeof(vector));
                break;
            }

            case KEELS2_CS2_VARIANT_ANGLES: {
                const QAngle angles(copy.vector_value[0],copy.vector_value[1],copy.vector_value[2]);
                CopyPayload(*result, FIELD_QANGLE, &angles, sizeof(angles));
                break;
            }
            ...
            case KEELS2_CS2_VARIANT_COLOR: {
                ...
                CopyPayload(*result, FIELD_COLOR32, &color, sizeof(color));
                break;
            }

            case KEELS2_CS2_VARIANT_ENTITY: {
                *result = CEntityHandle(copy.entity_handle);
                break;
            }
        }
...
extern "C" void KeelCs2Variant_Release(void* value)
{
    delete static_cast<CVariant*>(value);
}
...
extern "C" KeelResult KeelCs2Variant_Read(const void* pointer, KeelCs2VariantSnapshot* output)
{
    ...
    *output = {};
    ...
    result.native_type = output->native_type;

    switch (value.m_type) {
    case FIELD_VOID:
        result.type = KEELS2_CS2_VARIANT_VOID;
        break;
    ...
    case FIELD_STRING:
    {
        ...
        if (text)
        {
            ...
            while (length <= KEELS2_CS2_VARIANT_MAX_STRING && text[length])
                ++length;

            if (length > KEELS2_CS2_VARIANT_MAX_STRING)
        ...
        case FIELD_BOOLEAN: {
            ...
            result.int_value = boolean;
            break;
        }

        case FIELD_INT32:
            result.type = KEELS2_CS2_VARIANT_INT32;
            result.int_value = value.m_int32;
            break;

        case FIELD_FLOAT32:
            result.type = KEELS2_CS2_VARIANT_FLOAT;
            result.float_value = value.m_float32;

            if (!std::isfinite(result.float_value))
                return KEEL_RESULT_INCOMPATIBLE;
        ...
        case FIELD_QANGLE:
            ...
            for (float component : result.vector_value)
                if (!std::isfinite(component))
                    return KEEL_RESULT_INCOMPATIBLE;
        ...
        case FIELD_COLOR32:
            ...
            for (int i = 0; i < 4; ++i)
                result.color_value[i] = (*value.m_pColor)[i];

            break;

        case FIELD_EHANDLE:
            result.type = KEELS2_CS2_VARIANT_ENTITY;
            result.entity_handle = static_cast<uint32_t>(value.m_hEntity.ToInt());
            break;
