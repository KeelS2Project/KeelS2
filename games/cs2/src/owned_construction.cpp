...
OwnedConstructions::~OwnedConstructions() { Reset(); }
...
{
    if (token) for (const auto& record : records_) if (record && !record->closed && record->token == token) return record;
    return {};
}
unsigned OwnedConstructions::Count() const noexcept
{
    return static_cast<unsigned>(std::count_if(records_.begin(),records_.end(),[](const auto& record) { return !!record; }));
}
void OwnedConstructions::Detach(const std::shared_ptr<Record>& record) noexcept
{
    record->closed = true;
    for (auto& slot : records_) if (slot == record) { slot.reset(); break; }
}
KeelResult OwnedConstructions::Finish(const std::shared_ptr<Record>& record) noexcept
{
    if (record->consumed || !record->created) return KEEL_RESULT_OK;
    record->consumed = true;
    try { return backend_.Cancel(record->identity); }
...
KeelResult OwnedConstructions::Begin(std::uint64_t token, std::shared_ptr<Record>& record)
{
    ...
    if (resetting_ || record->busy || depth_ >= 8) return KEEL_RESULT_BUSY;
...
KeelResult OwnedConstructions::Build(const Record& record, const std::vector<Key>& keys, Values& output)
{
    ...
    for (std::size_t i = 0; i < keys.size(); ++i) views[i] = keys[i].View();
    void* value{};
    const auto result = KeelCs2KeyValues_Build(record.class_name.c_str(),views.data(),static_cast<std::uint32_t>(keys.size()),&value);
    output.reset(value);
    return result;
}
KeelResult OwnedConstructions::CopyKey(const KeelCs2EntityKeyValue& input, Key& key)
{
    if (input.size != sizeof(input) || !CopyText(input.name,KEELS2_CS2_KEY_MAX_NAME,key.name)) return KEEL_RESULT_INVALID_ARGUMENT;
    key.value = input;
    key.value.name = key.value.string_value = nullptr;
    if (input.type == KEELS2_CS2_KEY_STRING && !CopyText(input.string_value,KEELS2_CS2_KEY_MAX_STRING,key.text)) return KEEL_RESULT_INVALID_ARGUMENT;
    return KEEL_RESULT_OK;
}
KeelResult OwnedConstructions::Create(const char* class_name, std::uint64_t& token, host::GameEntityIdentity& identity) noexcept
{
    token = 0; identity = {};
    try {
        std::string name;
        if (!CopyText(class_name,KEELS2_CS2_KEY_MAX_NAME,name)) return KEEL_RESULT_INVALID_ARGUMENT;
        const auto ready = backend_.Ready();
        if (ready != KEEL_RESULT_OK) return ready;
        if (resetting_ || depth_ >= 8 || next_token_ == UINT64_MAX) return KEEL_RESULT_BUSY;
        auto slot = std::find(records_.begin(),records_.end(),nullptr);
        if (slot == records_.end()) return KEEL_RESULT_BUSY;
        auto record = std::make_shared<Record>(); record->class_name = std::move(name); record->token = next_token_++;
        ...
        *slot = record;
        ...
        token = record->token; identity = record->identity;
        return KEEL_RESULT_OK;
    } catch (...) { return KEEL_RESULT_ENGINE_FAILURE; }
}
KeelResult OwnedConstructions::Describe(std::uint64_t token, host::GameEntityIdentity& identity) noexcept
{
    ...
    try {
        ...
        if (result == KEEL_RESULT_OK && !record->closed) identity = record->identity;
...
KeelResult OwnedConstructions::Set(std::uint64_t token, const KeelCs2EntityKeyValue& input) noexcept
{
    try {
        ...
        auto found = std::find_if(keys.begin(),keys.end(),[&](const auto& old) { return SameKey(old.name,key.name); });
        if (found != keys.end()) *found = std::move(key);
        else {
            if (keys.size() >= KEELS2_CS2_KEY_MAX_COUNT) return KEEL_RESULT_BUSY;
            keys.push_back(std::move(key));
        }
        Values value;
        result = Build(*record,keys,value);
        if (result != KEEL_RESULT_OK || record->closed) return record->closed ? KEEL_RESULT_NOT_FOUND : result;
        record->keys = std::move(keys); record->values = std::move(value);
        return KEEL_RESULT_OK;
    } catch (...) { return KEEL_RESULT_ENGINE_FAILURE; }
}
KeelResult OwnedConstructions::Teleport(std::uint64_t token, const KeelEntityTeleport& input) noexcept
{
    if (input.size != sizeof(input) || !input.flags || (input.flags & ~7u)) return KEEL_RESULT_INVALID_ARGUMENT;
...
KeelResult OwnedConstructions::Spawn(std::uint64_t token, KeelBool& invoked) noexcept
{
    ...
    try {
        ...
        result = backend_.Validate(record->identity);
        if (result != KEEL_RESULT_OK || record->closed) return record->closed ? KEEL_RESULT_NOT_FOUND : result;
        try { result = backend_.Spawn(record->identity,record->values.get(),invoked); }
        catch (...) { result = KEEL_RESULT_ENGINE_FAILURE; }
        if (invoked) { record->consumed = true; Detach(record); }
        return result;
    } catch (...) { return KEEL_RESULT_ENGINE_FAILURE; }
}
KeelResult OwnedConstructions::Cancel(std::uint64_t token) noexcept
{
    try {
        ...
        if (!record) return KEEL_RESULT_NOT_FOUND;
        Detach(record);
        if (record->busy) return KEEL_RESULT_OK;
        Operation operation(*this,record);
...
void OwnedConstructions::Reset() noexcept
{
    ...
    records_ = {};
    for (auto& record : old) if (record) record->closed = true;
