    ...
    KeelCs2EntityInputBindings& bindings, std::string& error)
{
...
#endif
    if (profile != supported) { error = "entity inputs are unavailable for this compatibility profile"; return KEEL_RESULT_UNSUPPORTED; }
    platform::FileFingerprint fingerprint;
    if (!platform::FingerprintFile(module.path,fingerprint,error) || fingerprint != expected)
    { error = "entity input server fingerprint changed"; return KEEL_RESULT_INCOMPATIBLE; }
    ...
    for (std::size_t i = 0; i < entries.size(); ++i) {
        ...
        { error = "entity input address is outside the module"; return KEEL_RESULT_INCOMPATIBLE; }
        for (std::size_t j = 0; j < entry.bytes.size(); ++j)
            if (!platform::IsExecutableAddress(module,reinterpret_cast<void*>(base+entry.rva+j)))
            { error = "entity input code is not readable and executable"; return KEEL_RESULT_INCOMPATIBLE; }
        functions[i] = reinterpret_cast<void*>(base+entry.rva);
        if (std::memcmp(functions[i],entry.bytes.data(),entry.bytes.size()))
        { error = "entity input code changed"; return KEEL_RESULT_INCOMPATIBLE; }
    }
