    ...
    KeelCs2EntityToolBindings& bindings, std::string& error)
{
    ...
    {
        error = "entity tools are unavailable for this compatibility profile";
    ...
    {
        error = "entity tools server fingerprint changed";
    ...
    for (std::size_t i = 0; i < entries.size(); ++i) {
        ...
        {
            error = "entity tool address is outside the module";
            return KEEL_RESULT_INCOMPATIBLE;
        }
        ...
        for (std::size_t j = 0; j < entry.bytes.size(); ++j)
            ...
            {
                error = "entity tool code is not readable and executable";
                return KEEL_RESULT_INCOMPATIBLE;
            }
        ...
        {
            error = "entity tool code changed";
            return KEEL_RESULT_INCOMPATIBLE;
        }
    }
    ...
    if (platform::FindPrimaryVtable(module, "CBaseEntity", slot + 1, table, error) != platform::ModuleLookup::found ||
        table[slot] != functions[2])
    {
        error = "entity teleport virtual method is incompatible";
        return KEEL_RESULT_INCOMPATIBLE;
    }
    ...
    const KeelCs2EntityToolBindings& bindings, KeelCs2EntityToolClass& target, std::string& error)
{
    ...
    {
        error = "entity class teleport method is incompatible";
        return KEEL_RESULT_INCOMPATIBLE;
    }
