    ...
    Report& report)
{
    ...
    for (const Interface& interface_value : request.interfaces)
    {
        if (!modules.contains(interface_value.module))
        {
            Add(report,
                ...
                "interface " + interface_value.key + " names unknown module " + interface_value.module);
        }
    }
    ...
    for (const PatternRequest& pattern : request.patterns)
    {
        if (!modules.contains(pattern.module))
        {
            Add(report,
                Severity::error,
                "request-pattern-module",
                "pattern " + pattern.key + " names unknown module " + pattern.module);
        }
    }
    ...
    for (const PatternRequest& request_pattern : request.patterns)
    {
        ...
        if (image == images.end())
        {
            Add(report,
                Severity::error,
                "pattern-module",
                request_pattern.key + ": module was not captured: " + request_pattern.module);
        ...
        if (!SelectMatch(matches, request_pattern.occurrence, selected, reason))
        {
            Add(report,
                Severity::error,
                "pattern-resolution",
                request_pattern.key + " " + reason + " in module " + request_pattern.module);
...
Report Compare(const Profile& accepted, const Profile& candidate)
{
    ...
    if (candidate.status != ProfileStatus::candidate_untrusted)
    {
        Add(report,
            Severity::error,
            "candidate-trust",
            "candidate input is not marked candidate-untrusted with review required");
    }
    ...
    if (accepted.game != candidate.game)
    {
        Add(report,
            Severity::error,
            "game-mismatch",
            "accepted game " + accepted.game + " differs from candidate game " + candidate.game);
    }
    ...
    if (accepted.platform != candidate.platform)
    {
        Add(report,
            Severity::error,
            "platform-mismatch",
            "accepted platform " + accepted.platform + " differs from candidate platform " + candidate.platform);
    }
    ...
    const auto accepted_modules = IndexBy(accepted.modules,
                                          [](const Module& value)
                                          {
                                              return value.role;
                                          });

    const auto candidate_modules = IndexBy(candidate.modules,
                                           [](const Module& value)
                                           {
                                               return value.role;
                                           });
    ...
    const auto accepted_interfaces = IndexBy(accepted.interfaces,
                                             [](const Interface& value)
                                             {
                                                 return value.key;
                                             });

    const auto candidate_interfaces = IndexBy(candidate.interfaces,
                                              [](const Interface& value)
                                              {
                                                  return value.key;
                                              });
    ...
    const auto slot_key = [](const Slot& value)
    {
        return value.interface_key + "." + value.method;
    };
    ...
    for (const auto& [key, value] : accepted_slots)
    {
        ...
        else if (value->index != other->second->index)
        {
            Difference(report,
                       "slot-changed",
                       key + " " + std::to_string(value->index) + " -> " + std::to_string(other->second->index));
        }
    }
    ...
    const auto accepted_patterns = IndexBy(accepted.patterns,
                                           [](const Pattern& value)
                                           {
                                               return value.key;
                                           });

    const auto candidate_patterns = IndexBy(candidate.patterns,
                                            [](const Pattern& value)
                                            {
                                                return value.key;
                                            });
...
Report Validate(const Profile& profile, const std::vector<ModuleInput>& bindings)
{
    ...
    const auto modules = IndexBy(profile.modules,
                                 [](const Module& value)
                                 {
                                     return value.role;
                                 });
    ...
    for (const auto& [role, module] : modules)
    {
        ...
        if (binding->second.filename().string() != module->name)
        {
            Add(report,
                Severity::error,
                "module-name",
                role + ": expected filename " + module->name + " but received " + binding->second.filename().string());
    ...
    for (const Pattern& pattern : profile.patterns)
    {
        ...
        if (image == images.end())
        {
            Add(report,
                Severity::error,
                "pattern-cross-module",
                pattern.key + ": declared module was not validated: " + pattern.module);
        ...
        if (!SelectMatch(matches, pattern.occurrence, selected, reason))
        {
            Add(report,
                Severity::error,
                "pattern-resolution",
                pattern.key + " " + reason + " in declared module " + pattern.module);
    ...
    if (report.Ok() && report.review_required)
    {
        Add(report,
            Severity::warning,
            "human-review-required",
            "validation proves evidence consistency but does not trust the candidate");
    }
